...
export async function fetchProducts(outletId) {
  const result = await fetchAndCache(`/api/pos/products?outletId=${outletId}`, (data) =>
    cacheProducts(outletId, data),
  );
  if (result) return result;
  const cached = await getCachedProducts(outletId);
  if (cached.length > 0) return { data: cached, offline: true };
  const fallback = await fetchFallback(`/api/pos/products?outletId=${outletId}`, (data) =>
    cacheProducts(outletId, data),
  );
  if (fallback) return fallback;
...
export async function fetchCategories(outletId) {
  const result = await fetchAndCache(`/api/pos/categories?outletId=${outletId}`, (data) =>
    cacheCategories(outletId, data),
  );
  if (result) return result;
  const cached = await getCachedCategories(outletId);
  if (cached.length > 0) return { data: cached, offline: true };
  const fallback = await fetchFallback(`/api/pos/categories?outletId=${outletId}`, (data) =>
    cacheCategories(outletId, data),
  );
  if (fallback) return fallback;
  return { data: [], offline: true };
...
export async function fetchTables(outletId) {
  const result = await fetchAndCache(`/api/pos/tables?outletId=${outletId}`, (data) =>
    cacheTables(outletId, data),
  );
  if (result) return result;
  const cached = await getCachedTables(outletId);
  if (cached.length > 0) return { data: cached, offline: true };
  const fallback = await fetchFallback(`/api/pos/tables?outletId=${outletId}`, (data) =>
    cacheTables(outletId, data),
  );
  if (fallback) return fallback;
  return { data: [], offline: true };
...
export async function fetchOrders(outletId, params = {}) {
  ...
  const result = await fetchAndCache(`/api/pos/orders?${qs}`, (data) =>
    Promise.all(data.map((o) => cacheOrder(o))),
  );
  if (result) return result;
