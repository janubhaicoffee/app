  ...
  if (loading) {
    return (
      <div className="pos-fullscreen">
        <div className="pos-top-bar"><h1>Shifts</h1></div>
  ...
  return (
    <div className="pos-fullscreen">
      ...
      <div className="pos-shifts-container" style={{ overflow: "auto", flex: 1 }}>
        {error && (
          <div style={{ padding: 8, background: "#ffebee", color: "#c62828", fontSize: 13, borderRadius: 6, marginBottom: 16 }}>
            {error}
        ...
        {currentShift ? (
          <form className="pos-form" onSubmit={handleCloseShift}>
            <h3>Close Shift</h3>
            <div style={{ fontSize: 14, marginBottom: 12, padding: 8, background: "var(--bg-color)", borderRadius: 4 }}>
            ...
            {parseFloat(closingCash) > 0 && (
              <div style={{ fontSize: 14, marginBottom: 12, padding: 8, borderRadius: 4, background: expectedCash !== parseFloat(closingCash) ? "#fff3e0" : "#e8f5e9" }}>
                {expectedCash !== parseFloat(closingCash) ? (
                  <span>Difference: <strong>₹{(parseFloat(closingCash) - expectedCash).toFixed(2)}</strong></span>
                ) : (
            ...
            <button type="submit" className="pos-btn danger" style={{ width: "100%" }} disabled={submitting}>
          ...
          <form className="pos-form" onSubmit={handleOpenShift}>
            ...
            <button type="submit" className="pos-btn primary" style={{ width: "100%" }} disabled={submitting}>
          ...
          {shiftHistory.length === 0 ? (
            ...
            <div>
              {shiftHistory.map((shift) => (
                <div key={shift.id} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid var(--border-color)", fontSize: 13 }}>
                  <div>
                    ...
                    <div style={{ color: "var(--text-secondary)", fontSize: 12 }}>
                      ...
                      {shift.closed_at && ` · Close: ${new Date(shift.closed_at).toLocaleTimeString()}`}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    ...
                    {shift.difference !== undefined && shift.difference !== null && (
                      <div style={{ fontSize: 12, color: parseFloat(shift.difference) !== 0 ? "#c62828" : "#2e7d32" }}>
                        {parseFloat(shift.difference) >= 0 ? "+" : ""}₹{parseFloat(shift.difference).toFixed(2)}
                      </div>
                    )}
                  </div>
                </div>
