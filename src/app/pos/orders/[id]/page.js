...
export default function PosOrderDetail() {
  ...
  const updateItemStatus = async (itemId, newStatus) => {
    setUpdating(true);
    try {
      const result = await updateOrderStatus(params.id, {
        item_id: itemId,
  ...
  if (error || !order) {
    return (
      <div className="pos-fullscreen">
        <div className="pos-top-bar">
          <button onClick={() => router.push('/pos/orders')}>
    ...
    <div className="pos-fullscreen">
      ...
      <div className="pos-order-detail" style={{ overflow: 'auto', flex: 1 }}>
        <div className="pos-detail-header">
          <div>
            <h2 style={{ margin: 0, fontSize: 20 }}>{order.customer_name || 'Walk-in Customer'}</h2>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <span
              className={`pos-badge ${order.status}`}
        ...
        >
          ...
          <div className="pos-total-row grand-total" style={{ fontSize: 20 }}>
            <span>Total</span>
        ...
        <div className="pos-detail-actions">
          ...
          {order.status === 'preparing' && (
            <button
              className="pos-btn secondary"
              onClick={() => handleOrderStatusUpdate('ready')}
              disabled={updating}
            >
              Mark Ready
            </button>
