      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            ...
            setOrders((prev) =>
              prev.filter((o) => o.id !== payload.new?.id && o.id !== payload.old?.id),
            );
            return;
          }
  ...
  if (loading) {
    return (
      <div className="pos-fullscreen">
        <div className="pos-loading" style={{ background: 'var(--bg-color-dark)' }}>
        ...
        {orders.length === 0 ? (
          ...
          orders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className={`pos-kitchen-card ${order.status} ${isNew ? 'new-order' : ''}`}
              >
                <div className="pos-kitchen-header">
                  <div className="pos-kitchen-order-no">
                    #{order.order_number || order.id.toString().slice(-4)}
                  </div>
                  <div className="pos-kitchen-time">{elapsed(order.created_at)}</div>
                </div>
                ...
                <div className="pos-kitchen-actions">
                  {order.status === 'pending' && (
                    <button
                      className="pos-kitchen-btn start"
                  ...
                  {order.status === 'preparing' && (
                    <button
                      className="pos-kitchen-btn ready-btn"
