...
export default function PosNewOrder() {
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'pos_tables', filter: `outlet_id=eq.${outlet.id}` },
        () => {
          loadCatalog();
        },
      )
      ...
      <div className="pos-order-type-bar">
        ...
        {['dine-in', 'takeaway', 'delivery'].map((type) => (
          <button
            ...
            onClick={() => {
              setOrderType(type);
              if (type !== 'dine-in') setShowTableSelector(false);
            }}
          >
        ...
        {orderType === 'dine-in' && (
          <button
            ...
            style={{
              marginLeft: 'auto',
              borderColor: selectedTable ? 'var(--accent-red)' : undefined,
            }}
