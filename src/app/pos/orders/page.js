  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      if (sourceFilter !== 'all') {
        ...
        params.status =
          activeTab === 'active' ? 'pending,preparing,ready' : 'completed,cancelled,served';
      }
    ...
    <div className="pos-fullscreen">
      ...
      <div className="pos-orders-container" style={{ overflow: 'auto', flex: 1 }}>
        <div className="pos-tabs">
          ...
          <button
            className={`pos-tab ${activeTab === 'history' ? 'active' : ''}`}
            onClick={() => {
        ...
        >
          {['all', 'qr_menu', 'pos', 'online'].map((src) => (
            <button
              key={src}
              className={`pos-source-btn ${sourceFilter === src ? 'active' : ''}`}
              onClick={() => {
                setSourceFilter(src);
                setLoading(true);
              }}
        ...
        {loading ? (
          ...
          <div className="pos-empty">
            ...
            {activeTab === 'active' && (
              <button
                ...
                style={{ marginTop: 12 }}
                onClick={() => router.push('/pos/orders/new')}
              >
                Create New Order
              </button>
            )}
          ...
          orders.map((order) => (
            ...
            >
              <div className="pos-order-number">
              ...
              <div className="pos-order-info">
                ...
                <div className="pos-order-meta">
                  {order.source && (
                    <span className={`pos-source-badge ${sourceLabel(order.source).cls}`}>
                      {sourceLabel(order.source).label}
                    </span>
                  )}
