  ...
  return (
    <div className="pos-page">
      ...
      <div className="pos-header">
        ...
        <button className="pos-btn-outline" onClick={fetchPickups}>
          <RefreshCw size={16} />
        </button>
      </div>
      ...
      <div className="pos-stats-grid">
        <div className="pos-stat-card">
          <div className="pos-stat-value" style={{ color: '#e65100' }}>
            {pendingCount}
          </div>
          <div className="pos-stat-label">Pending / Active</div>
        </div>
        <div className="pos-stat-card">
          <div className="pos-stat-value" style={{ color: '#004085' }}>
            {activeCount}
          </div>
          <div className="pos-stat-label">In Progress</div>
        </div>
        <div className="pos-stat-card">
          <div className="pos-stat-value" style={{ color: '#155724' }}>
        ...
        >
          {pickups.map((pu) => {
            ...
            return (
              <div
                key={pu.id}
                className="pos-order-card pickup-card"
                style={{
              ...
              >
                ...
                {pu.items_summary?.length > 0 && (
                  <div style={{ fontSize: '0.8rem', color: '#718096', marginBottom: '0.75rem' }}>
                    {Array.isArray(pu.items_summary) ? (
                      pu.items_summary.map((item, i) => (
                        <span key={i}>
                          {item.name || item} x{item.quantity || 1}
                          {i < pu.items_summary.length - 1 ? ', ' : ''}
                        </span>
                      ))
                    ) : (
                      <span>
                        {typeof pu.items_summary === 'string'
                          ? pu.items_summary
                          : JSON.stringify(pu.items_summary)}
                      </span>
                    )}
                  </div>
                )}
