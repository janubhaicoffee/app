...
import { Package, Search, Calendar, Landmark, MapPin, Truck, CheckCircle2, ChevronRight } from "lucide-react";
import "./page.css";
...
function TrackPageContent() {
  ...
  useEffect(() => {
    ...
    const orderParam = searchParams.get("order") || searchParams.get("awb") || searchParams.get("q");
    if (orderParam) {
      setQuery(orderParam);
  ...
  const getStepperStage = (status) => {
    ...
    if (s === "shipped" || s === "in_transit" || s === "dispatched" || s === "out_for_delivery" || s === "rto" || s === "returned") return 4;
  ...
  return (
    <main className="track-page-wrapper">
      <div className="track-container vintage-border">
        ...
        <p className="track-subtitle" style={{ textAlign: "center", marginBottom: "2rem", color: "var(--text-secondary)" }}>
          Enter your Order Number (e.g. JB-1720367489), AWB, or Database UUID.
        </p>

        <form onSubmit={handleTrack} className="track-form" style={{ display: "flex", gap: "10px", marginBottom: "2rem" }}>
          <div style={{ position: "relative", flex: 1 }}>
            <input
              ...
              required
            />
            <Package size={20} style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", color: "#888" }} />
          </div>
          <button type="submit" className="track-btn" style={{ display: "flex", alignItems: "center", gap: "8px" }} disabled={loading}>
            <Search size={18} />
        ...
        {loading && (
          <div className="text-center" style={{ padding: "3rem 0" }}>
            <div className="spinner" style={{ border: "4px solid #f3f3f3", borderTop: "4px solid var(--accent-red)", borderRadius: "50%", width: "40px", height: "40px", animation: "spin 1s linear infinite", margin: "0 auto 15px" }}></div>
            <p style={{ color: "var(--text-secondary)" }}>Retrieving live order and shipping status...</p>
          </div>
        )}
          ...
          <div className="track-results-content fade-in">
            ...
            <div className="stepper-wrapper" style={{ margin: "2rem 0 3rem 0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", position: "relative" }}>
                <div style={{ position: "absolute", top: "18px", left: "10%", right: "10%", height: "4px", backgroundColor: "#e2e8f0", zIndex: 1 }}></div>
                <div 
                  style={{ 
                    position: "absolute", 
                    top: "18px", 
                ...
                {stepperStages.map((stage) => {
                  ...
                  return (
                    <div key={stage.number} style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "20%", zIndex: 3 }}>
                      <div 
                        style={{ 
                          width: "40px", 
                          height: "40px", 
                      ...
                      <span style={{ fontSize: "0.85rem", fontWeight: isCompleted ? "bold" : "normal", marginTop: "8px", color: isCompleted ? "var(--text-primary)" : "#64748b" }}>{stage.label}</span>
                      <span style={{ fontSize: "0.7rem", color: "#94a3b8", display: "block", textAlign: "center" }}>{stage.desc}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="track-result" style={{ margin: 0 }}>
                ...
                <div className="track-info-grid" style={{ gridTemplateColumns: "1fr" }}>
                  <div className="track-info-item">
                    ...
                    <span className="track-value" style={{ fontFamily: "monospace", fontSize: "1.2rem", color: "var(--accent-red)" }}>{trackingData.order_number}</span>
                  </div>
                  {orderData && (
                    <>
                      <div className="track-info-item" style={{ marginTop: "10px" }}>
                        <span className="track-label">Order Date</span>
                        <span className="track-value" style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                          <Calendar size={16} />
                      ...
                      <div className="track-info-item" style={{ marginTop: "10px" }}>
                        ...
                        <span className="track-value" style={{ fontWeight: "700" }}>₹ {orderData.total_amount}</span>
                      </div>
                    </>
                  )}
                </div>
              ...
              <div className="track-result" style={{ margin: 0 }}>
                ...
                <div className="track-info-grid" style={{ gridTemplateColumns: "1fr" }}>
                  <div className="track-info-item">
                    ...
                    <span className="track-value" style={{ fontFamily: "monospace" }}>{trackingData.awb_number || "Awaiting courier assignment"}</span>
                  </div>
                  <div className="track-info-item" style={{ marginTop: "10px" }}>
            ...
            {orderData && orderData.items && orderData.items.length > 0 && (
              <div className="track-result" style={{ marginBottom: "2rem" }}>
                <h3 style={{ borderBottom: "1px solid #e2e8f0", paddingBottom: "8px", marginBottom: "15px" }}>Items in this Shipment</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  {orderData.items.map((item, idx) => (
                    <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <span style={{ backgroundColor: "var(--accent-red)", color: "white", padding: "2px 8px", borderRadius: "4px", fontSize: "0.85rem", fontWeight: "bold" }}>{item.quantity}x</span>
                        <span style={{ fontWeight: "500" }}>{item.product_name}</span>
                      </div>
                      <span style={{ fontWeight: "bold", color: "#64748b" }}>₹ {item.price * item.quantity}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="track-result">
              ...
              <div className="timeline-container" style={{ paddingLeft: "15px", marginTop: "1.5rem" }}>
                {trackingData.history && trackingData.history.length > 0 ? (
                  trackingData.history.map((event, index) => (
                    <div key={index} className="timeline-event" style={{ paddingLeft: "15px" }}>
                      <div className="timeline-dot" style={{ backgroundColor: index === 0 ? "var(--accent-red)" : "#94a3b8", boxShadow: index === 0 ? "0 0 10px var(--accent-red)" : "none" }}></div>
                      <div className="timeline-content">
                        <h3 style={{ color: index === 0 ? "var(--accent-red)" : "var(--text-primary)", fontWeight: index === 0 ? "700" : "500", fontSize: "1.05rem" }}>
                          {event.status_code} - {event.message}
                        </h3>
                        {event.location && <p style={{ fontSize: "0.85rem", margin: "2px 0 6px 0", color: "#64748b" }}><MapPin size={12} style={{ display: "inline", marginRight: "3px" }} /> {event.location}</p>}
                        <span className="timeline-date" style={{ color: index === 0 ? "var(--accent-red)" : "#64748b" }}>{event.event_time}</span>
                      </div>
                    </div>
                  ))
                ) : (
...
export default function TrackPage() {
  return (
    <Suspense fallback={<div className="track-page-wrapper"><div className="track-container text-center"><p>Loading tracking screen...</p></div></div>}>
      <TrackPageContent />
    </Suspense>
  );
}
