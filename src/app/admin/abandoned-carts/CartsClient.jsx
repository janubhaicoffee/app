              ...
              <div key={cart.id} className="cart-card">
                ...
                <div className="cart-customer">
                  {cart.customer_email ? (
                    ...
                    <div className="customer-info text-muted">No email captured (Guest)</div>
                  )}
                </div>

                <div className="cart-items">
                  <p className="items-title">
                    <ShoppingCart size={14} /> Items Left Behind:
                  </p>
                  <ul className="item-list">
                    {cart.cart_payload.map((item, idx) => (
                      <li key={idx}>
                        {item.quantity}x {item.name}{' '}
                        {item.variantSlug ? `(${item.variantSlug})` : ''}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                {cart.customer_email && cart.status === 'abandoned' && (
                  <div className="cart-actions">
                    <button
                      ...
                      onClick={() => {
                        window.open(
                          `mailto:${cart.customer_email}?subject=You left something behind!&body=Hi! We noticed you left some items in your cart. Here's a 10% discount code to complete your order: COMEBACK10`,
                        );
                      }}
                    >
