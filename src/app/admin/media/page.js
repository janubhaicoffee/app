...
import { Grid, List, Upload, Trash2, Copy, Check, X, Image, FileVideo, FileText, Folder } from "lucide-react";
    ...
    <div className="admin-media-page">
      ...
      <div className="admin-toolbar">
        ...
        <select value={folderFilter} onChange={e => setFolderFilter(e.target.value)}
          style={{ padding: '0.4rem 0.6rem', borderRadius: '6px', border: '1px solid var(--border-color)', fontSize: '0.82rem', background: '#fff' }}>
          {folders.map(f => (
        ...
        <div style={{ display: 'flex', gap: '4px', border: '1px solid var(--border-color)', borderRadius: '6px', overflow: 'hidden' }}>
          <button className={`admin-btn-sm ${viewMode === 'grid' ? 'active-view' : ''}`}
            onClick={() => setViewMode('grid')} style={{ borderRadius: 0, border: 'none', background: viewMode === 'grid' ? 'var(--primary-color)' : '#fff', color: viewMode === 'grid' ? '#fff' : 'var(--text-secondary)' }}>
            <Grid size={14} />
          </button>
          <button className={`admin-btn-sm ${viewMode === 'list' ? 'active-view' : ''}`}
            onClick={() => setViewMode('list')} style={{ borderRadius: 0, border: 'none', background: viewMode === 'list' ? 'var(--primary-color)' : '#fff', color: viewMode === 'list' ? '#fff' : 'var(--text-secondary)' }}>
            <List size={14} />
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="admin-loading"><div className="admin-spinner"></div> Loading media...</div>
      ) : filteredMedia.length === 0 ? (
        <div className="admin-card">
          <div className="empty-state">
            <Image size={48} />
            <h3>No media files found</h3>
            <p>{media.length === 0 ? 'Add your first media file to get started.' : 'No files match your filters.'}</p>
          </div>
        </div>
      ) : viewMode === 'grid' ? (
            ...
            return (
              <div key={item.id} className="media-grid-item">
                <div className="media-thumb">
                  {isImage ? (
                    <img src={item.url} alt={item.name || ''} loading="lazy"
                      onError={e => { e.target.style.display = 'none'; e.target.parentElement.innerHTML = '<div class=\"media-fallback\"><svg width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\"><rect x=\"3\" y=\"3\" width=\"18\" height=\"18\" rx=\"2\" ry=\"2\"/><circle cx=\"8.5\" cy=\"8.5\" r=\"1.5\"/><polyline points=\"21 15 16 10 5 21\"/></svg></div>'; }} />
                  ) : isVideo ? (
                    <div className="media-fallback"><FileVideo size={28} /></div>
                  ) : (
                    <div className="media-fallback"><FileText size={28} /></div>
                  )}
                  <div className="media-actions-overlay">
                    <button className="media-action-btn" title="Copy URL" onClick={() => copyUrl(item)}>
                      {copiedId === item.id ? <Check size={14} /> : <Copy size={14} />}
                    </button>
                    <button className="media-action-btn media-action-btn-danger"
                ...
                <div className="media-info">
                  <p className="media-name" title={item.name || 'Untitled'}>{item.name || 'Untitled'}</p>
                  <p className="media-meta">{formatFileSize(item.size)} · {inferredType}</p>
                </div>
              </div>
            );
          })}
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '4px' }}>
                        <button className="admin-btn-sm-icon" title="Copy URL" onClick={() => copyUrl(item)}>
                          {copiedId === item.id ? <Check size={13} /> : <Copy size={13} />}
                        </button>
                        <button className={`admin-btn-sm-icon ${confirmDelete?.id === item.id ? 'btn-confirming' : ''}`}
                          title="Delete" onClick={() => handleDelete(item)}>
        ...
        <div className="modal-overlay" onClick={() => setShowUploadModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                <label>Folder</label>
                <input type="text" value={fileFolder} onChange={e => setFileFolder(e.target.value)}
                  placeholder="e.g. products, banners, blog" />
                <span className="form-hint">Organize media into folders (optional)</span>
              </div>
            </div>
            <div className="modal-footer">
              <button className="admin-btn-outline" onClick={() => setShowUploadModal(false)}>Cancel</button>
              <button className="admin-btn" onClick={handleUpload} disabled={uploading || !url.trim()}>
      ...
      <style jsx global>{`
        .admin-media-page { position: relative; }
        .media-grid {
          display: grid;
        ...
        .media-grid-item:hover .media-actions-overlay { opacity: 1; }
        .media-action-btn {
        ...
        .media-action-btn:hover { background: #fff; transform: scale(1.1); }
        .media-action-btn-danger:hover { background: #c62828; color: #fff; }
        .media-info {
        ...
        .media-list-thumb svg { color: #bbb; }
        .admin-btn-sm-icon {
        ...
        .admin-btn-sm-icon:hover { border-color: var(--primary-color); color: var(--primary-color); }
        ...
        .active-view { background: var(--primary-color) !important; color: #fff !important; border-color: var(--primary-color) !important; }
      `}</style>
    </div>
  );
}
