...
export default function AdminPartners() {
  ...
  const [form, setForm] = useState({
    email: "", name: "", outlet_id: "", role: "owner", stake: ""
  });
  ...
  if (loading) {
    return <div className="admin-loading"><div className="admin-spinner" /> Loading partners...</div>;
  }
    ...
    <div>
      ...
      {filteredPartners.length === 0 ? (
        <div className="empty-state">
          ...
          <p>{search ? "Try a different search term" : "Add business partners to manage outlet ownership"}</p>
          {!search && (
            <button className="admin-btn" onClick={() => setShowModal(true)}>
              <Plus size={16} /> Add Partner
            </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: '1.25rem' }}>
          {filteredPartners.map(partner => {
            ...
            try {
              if (partner.notes) {
                const parsed = typeof partner.notes === 'string' ? JSON.parse(partner.notes) : partner.notes;
              ...
              <div key={partner.id} className="admin-card">
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
                  <div style={{
                    ...
                    fontWeight: 700, fontSize: '1rem', color: badge.color
                  }}>
                    {partner.name?.charAt(0) || '?'}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>{partner.name || 'Unnamed'}</div>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    {stake > 0 && (
                      <div style={{ marginTop: '0.25rem', fontSize: '0.8rem', fontWeight: 700, color: stake >= 50 ? '#2e7d32' : 'var(--text-secondary)' }}>
                ...
                <div style={{ padding: '0.5rem 0', borderTop: '1px solid var(--border-color)', fontSize: '0.85rem' }}>
                  <div style={{ color: 'var(--text-secondary)', marginBottom: '0.3rem' }}>Outlets Managed:</div>
                  {uniqueOutlets.length === 0 ? (
                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>Not assigned to any outlet</span>
                  ) : (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
                      {uniqueOutlets.map(oid => (
                        <span key={oid} className="status-badge" style={{
                          background: '#f0f0f0', color: 'var(--primary-color)', fontWeight: 600
                        }}>
                ...
                <div style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                  <span>Created: {partner.created_at ? new Date(partner.created_at).toLocaleDateString() : '—'}</span>
                  {stake >= 50 && (
            ...
            <form onSubmit={handleAddPartner}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  <select required value={form.outlet_id}
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <input type="number" min="0" max="100" value={form.stake}
                ...
                {parseFloat(form.stake) >= 50 && (
                  <div style={{
                    padding: '0.75rem', background: '#e8f5e9', borderRadius: 6,
                    border: '1px solid #c8e6c9', fontSize: '0.85rem', color: '#2e7d32', fontWeight: 600
                  }}>
