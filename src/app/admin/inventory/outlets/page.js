...
export default function OutletInventoryPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (prodRes.ok) { const j = await prodRes.json(); setProducts(j.data || []); }
      if (outletRes.ok) { const j = await outletRes.json(); setOutlets(j.data || []); }
      setLoading(false);
    };
    load();
  }, []);
  ...
  const handleTransfer = async (productId, outletId, quantity, notes) => {
    ...
    try {
      const res = await fetch("/api/admin/inventory", {
        ...
        body: JSON.stringify({ action: "transfer_to_outlet", product_id: productId, outlet_id: outletId, quantity, notes })
      });
      if (res.ok) {
  ...
  const handleAdjust = async (productId, newStock, notes) => {
    ...
    try {
      const res = await fetch("/api/admin/inventory", {
        method: "POST",
        headers: { "Authorization": `Bearer ${session.access_token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ action: "adjust_stock", product_id: productId, new_stock: newStock, notes })
      });
      if (res.ok) {
    ...
    <div>
      ...
      <div className="admin-toolbar">
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <select value={filterOutlet} onChange={e => setFilterOutlet(e.target.value)}
            style={{ padding: "0.5rem", borderRadius: "6px", border: "1px solid var(--border-color)", fontSize: "0.85rem" }}>
            <option value="">All Outlets</option>
            {outlets.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
          </select>
        </div>
        <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>
          {filtered.length} product(s) across {new Set(filtered.map(p => p.outlet_id)).size} outlet(s)
        </span>
      </div>
      ...
      <div className="admin-card" style={{ padding: 0 }}>
        <table className="admin-table">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--text-secondary)" }}>No products found</td></tr>
            ) : filtered.map(p => {
              const stockStatus = p.current_stock <= 0 ? "out_of_stock" : p.current_stock <= (p.min_stock || 5) ? "low_stock" : "in_stock";
              const statusColor = stockStatus === "out_of_stock" ? "#c62828" : stockStatus === "low_stock" ? "#e65100" : "#2e7d32";
              return (
                <tr key={`${p.outlet_id}-${p.id}`}>
                  <td style={{ fontWeight: 600 }}>{p.name}</td>
                  <td style={{ fontSize: "0.85rem" }}>{p.outlet?.name || "Unknown"}</td>
                  <td style={{ fontSize: "0.8rem", color: "var(--text-secondary)", fontFamily: "monospace" }}>{p.sku || "-"}</td>
                  ...
                  <td>
                    <span style={{
                      ...
                      background: stockStatus === "out_of_stock" ? "#ffebee" : stockStatus === "low_stock" ? "#fff3e0" : "#e8f5e9",
                      color: statusColor
                    }}>
                      {stockStatus === "out_of_stock" ? "Out" : stockStatus === "low_stock" ? "Low" : "In Stock"}
                    </span>
                  </td>
                  <td>
                    <div style={{ display: "flex", gap: "0.25rem" }}>
                      <button className="admin-btn outline sm" onClick={() => setTransferModal(p)}>
      ...
      {transferModal && (
        <ModalForm
          ...
          onSave={(vals) => handleTransfer(transferModal.id, transferModal.outlet_id, vals.quantity, vals.notes)}
      ...
      {adjustModal && (
        <ModalForm
          ...
          fields={[
            ...
            { label: "New Stock", type: "number", key: "newStock", defaultValue: String(adjustModal.current_stock ?? 0) },
            { label: "Notes", type: "text", key: "notes", defaultValue: "" }
          ]}
...
function ModalForm({ title, fields, onSave, onCancel, saving }) {
  const [values, setValues] = useState(() => {
    const initial = {};
    fields.forEach(f => { if (f.key) initial[f.key] = f.defaultValue || ""; });
    return initial;
  });
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#fff", borderRadius: "12px", padding: "1.5rem", minWidth: 380,
        maxWidth: 450, boxShadow: "0 8px 32px rgba(0,0,0,0.15)"
      }} onClick={e => e.stopPropagation()}>
        <h3 style={{ margin: "0 0 1rem" }}>{title}</h3>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {fields.map((f, i) => (
            <div key={i}>
              <label style={{ fontSize: "0.8rem", fontWeight: 600, color: "var(--text-secondary)", display: "block", marginBottom: "0.25rem" }}>{f.label}</label>
              {f.readOnly ? (
                <div style={{ padding: "0.5rem", background: "#f5f5f5", borderRadius: "6px", fontSize: "0.9rem", fontWeight: 600 }}>{f.value}</div>
              ) : (
                <input
                  type={f.type || "text"}
                  value={values[f.key] || ""}
          ...
          <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end", marginTop: "0.5rem" }}>
            <button type="button" onClick={onCancel} className="admin-btn outline" disabled={saving}>Cancel</button>
