...
import {
  ArrowLeft, DollarSign, ShoppingCart, Users, Clock, TrendingUp,
  BarChart3, Store, MapPin, Phone, Mail, Building2, Edit3,
  CreditCard, UserPlus, Receipt, Activity, CheckCircle, Link2, Unlink, Search
} from "lucide-react";
import {
...
export default function OutletDetail() {
  ...
  async function loadOutletData() {
    try {
      ...
      if (ordersRes.ok) {
        ...
        const outletOrders = (json.data || []).filter(o =>
          o.outlet_id === params.id || o.outlet_code === outlet?.code
        );
  ...
  if (loading) {
    return <div className="admin-loading"><div className="admin-spinner" /> Loading outlet details...</div>;
  }
  ...
  if (error || !outlet) {
    return (
      <div className="empty-state">
        ...
        <Link href="/admin/outlets" className="admin-btn-outline">Back to Outlets</Link>
      </div>
    );
  }
  ...
  const monthlyExpenses = (parseFloat(settings.rent || 0) || 0) +
    ...
    <div>
      <div className="admin-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <p style={{ margin: '0.25rem 0 0', color: 'var(--text-secondary)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <MapPin size={12} /> {outlet.city || outlet.address || "Location not set"} &middot; Code: {outlet.code}
            </p>
          </div>
        </div>
      ...
      <div className="stats-grid">
        <div className="stat-card green">
          <h3><DollarSign size={14} style={{ display: 'inline', marginRight: 4 }} /> Today's Revenue</h3>
          <p className="stat-value">₹ {todayRevenue.toLocaleString('en-IN')}</p>
          <p className="stat-sub">{todayOrders.length} orders today</p>
        </div>
        <div className="stat-card blue">
          <h3><TrendingUp size={14} style={{ display: 'inline', marginRight: 4 }} /> MTD Revenue</h3>
        ...
        <div className="stat-card gold">
          <h3><Users size={14} style={{ display: 'inline', marginRight: 4 }} /> Staff</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
            <div className="admin-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', padding: '0.35rem 0', borderBottom: '1px solid var(--border-color)' }}>
                ...
                {settings.manager_name && (
                  <>
                    <div style={{ borderTop: '2px solid var(--border-color)', margin: '0.5rem 0', paddingTop: '0.5rem' }}>
                      <strong style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Manager</strong>
                    </div>
            ...
            <div className="admin-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <Link href={`/`} className="admin-btn-outline" style={{ justifyContent: 'center', width: '100%' }}>
                  <Store size={16} /> View Outlet Dashboard
                </Link>
                <Link href={`/`} className="admin-btn-outline" style={{ justifyContent: 'center', width: '100%' }}>
                  <CreditCard size={16} /> View POS
                </Link>
                <button className="admin-btn-outline" style={{ justifyContent: 'center', width: '100%' }}
      ...
      {activeTab === "orders" && (
        <div className="admin-card">
          <div className="admin-card-header">
            <h2>Recent Orders</h2>
            <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{orders.length} total</span>
          </div>
          {orders.length === 0 ? (
            <div className="empty-state">
                  ...
                  {orders.slice(0, 50).map((order, i) => (
                    <tr key={order.id || i}>
                      <td style={{ fontWeight: 600, fontFamily: 'monospace', fontSize: '0.8rem' }}>
                        #{order.id?.toString().slice(-6).toUpperCase() || `ORD-${String(i + 1).padStart(4, '0')}`}
                      </td>
                      <td>{order.customer_email || order.customer_name || 'Guest'}</td>
                      <td style={{ fontWeight: 600 }}>₹{Number(order.total_amount || 0).toLocaleString('en-IN')}</td>
                      <td>
                        <span className="status-badge" style={{
      ...
      {activeTab === "staff" && (
        <div className="admin-card">
          <div className="admin-card-header">
            <h2>Staff Members</h2>
            <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{staff.length} members</span>
          </div>
          {staff.length === 0 ? (
            <div className="empty-state">
              <Users size={40} />
              ...
              {staff.map(member => {
                ...
                return (
                  <div key={member.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <div style={{
                        width: 36, height: 36, borderRadius: '50%', background: badge.bg,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontWeight: 700, fontSize: '0.85rem', color: badge.color
                      }}>
                        {member.name?.charAt(0) || '?'}
                      </div>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{member.name}</div>
                        <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)' }}>{member.email || member.phone || '—'}</div>
                      </div>
                    </div>
        ...
        <>
          <div className="stats-grid">
            ...
            <div className="stat-card red">
              ...
              <p className="stat-value">₹ {Number(settings.electricity || 0).toLocaleString('en-IN')}</p>
            </div>
            <div className="stat-card red">
              <h3>Water + Internet</h3>
              <p className="stat-value">₹ {(Number(settings.water || 0) + Number(settings.internet || 0)).toLocaleString('en-IN')}</p>
            </div>
            <div className="stat-card red">
              ...
              <tbody>
                {[
                  ...
                  <tr key={i}>
                    ...
                    <td>{monthlyExpenses > 0 ? ((exp.amount / monthlyExpenses) * 100).toFixed(1) : 0}%</td>
                  </tr>
                ))}
                {monthlyExpenses === 0 && (
                  <tr><td colSpan={3} style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>No expenses configured</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function OutletSourcesTab({ outletId }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (ppRes.ok) { const j = await ppRes.json(); setPosProducts(j.data || []); }
      if (spRes.ok) { const j = await spRes.json(); setSourceProducts(j.data || []); }
      setLoading(false);
    };
    load();
  }, [outletId]);
  ...
  const handleUnlink = async (posProductId) => {
    ...
    try {
      const res = await fetch("/api/pos/products", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: posProductId, source_product_id: null, commission_per_unit: null })
      });
      if (res.ok) {
  ...
  const filteredSP = sourceProducts.filter(p =>
        ...
        <table className="admin-table">
          ...
          <tbody>
            {posProducts.length === 0 ? (
              <tr><td colSpan={5} style={{ textAlign: "center", color: "var(--text-secondary)" }}>No products in outlet menu</td></tr>
            ) : posProducts.map(pp => {
                  ...
                  <td>
                    {editingProduct === pp.id ? (
                      <SourceLinkForm
                        ...
                        onCancel={() => { setEditingProduct(null); setSearchQuery(""); }}
                        saving={saving}
                      />
                    ) : isLinked ? (
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", minWidth: 250 }}>
      ...
      <div style={{ display: "flex", gap: "0.25rem" }}>
        ...
        <button type="button" className="admin-btn outline sm" onClick={onCancel}>Cancel</button>
      </div>
    </form>
  );
}
...
function OutletCommissionsTab({ outletId }) {
  ...
  useEffect(() => { if (outletId) fetchData(); }, [outletId, filterStatus]);
  ...
  return (
    <div>
      ...
      <div className="admin-toolbar">
        ...
        <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>{commissions.length} transaction(s)</span>
      </div>
        ...
        <table className="admin-table">
          ...
          <tbody>
            {commissions.length === 0 ? (
              <tr><td colSpan={8} style={{ textAlign: "center", color: "var(--text-secondary)" }}>No commission transactions</td></tr>
            ) : commissions.map(c => (
              <tr key={c.id}>
                <td style={{ fontSize: "0.85rem", fontFamily: "monospace" }}>{c.pos_orders?.order_number || "-"}</td>
