...
import { Store, Plus, Search, Edit3, Power, PowerOff, MapPin, Users, DollarSign, X, Building2 } from "lucide-react";
...
export default function AdminOutlets() {
  ...
  const [form, setForm] = useState({
    ...
    rent: "", electricity: "", water: "", internet: "", cogs: ""
  });
  ...
  if (loading) {
    return <div className="admin-loading"><div className="admin-spinner" /> Loading outlets...</div>;
  }
    ...
    <div>
      ...
      <div className="admin-toolbar">
        <div className="admin-search">
          ...
          <input placeholder="Search by name, city or code..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          ...
          <p>{search ? "Try a different search term" : "Create your first outlet to get started"}</p>
          {!search && (
            <button className="admin-btn" onClick={openCreateModal}>
              <Plus size={16} /> Create Outlet
            </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1.25rem' }}>
          {filtered.map(outlet => {
            ...
            const staffCount = 0;
            return (
              <Link href={`/admin/outlets/${outlet.id}`} key={outlet.id} className="admin-card" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                    <div style={{
                      width: 40, height: 40, borderRadius: 8, background: 'var(--primary-color)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center'
                    }}>
                      <Building2 size={20} color="#fff" />
                    </div>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--primary-color)' }}>{outlet.name}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontFamily: 'monospace' }}>{outlet.code}</div>
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    ...
                    <span style={{ marginLeft: '0.5rem' }}>Electricity: ₹{Number(s.electricity || 0).toLocaleString('en-IN')}</span>
                  </div>
                </div>
                ...
                <div style={{ position: 'absolute', top: '0.75rem', right: '5rem', display: 'flex', gap: '0.25rem' }}>
                  <button onClick={e => { e.preventDefault(); e.stopPropagation(); openEditModal(outlet); }}
                    style={{ padding: '0.3rem', background: 'none', border: '1px solid var(--border-color)', borderRadius: 4, cursor: 'pointer' }}>
                    <Edit3 size={14} color="var(--text-secondary)" />
                  </button>
                  <button onClick={e => { e.preventDefault(); e.stopPropagation(); toggleStatus(outlet); }}
                    style={{ padding: '0.3rem', background: 'none', border: '1px solid var(--border-color)', borderRadius: 4, cursor: 'pointer' }}>
                    {outlet.status === "active" ? <PowerOff size={14} color="#c62828" /> : <Power size={14} color="#2e7d32" />}
                  </button>
                </div>
              </Link>
            );
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave}>
              <div className="modal-body">
                <h3 style={{ margin: '0 0 0.75rem', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>Basic Information</h3>
                <div className="form-row">
                  <div className="form-group">
                    <label>Outlet Name *</label>
                    <input required value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="Janu Bhai Coffee - Indira Nagar" />
                  </div>
                  <div className="form-group">
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    <label>GSTIN</label>
                    <input value={form.gstin} onChange={e => setForm({...form, gstin: e.target.value})} placeholder="29ABCDE1234F1Z5" />
                  </div>
                </div>

                <h3 style={{ margin: '1rem 0 0.75rem', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>Manager Details</h3>
                <div className="form-row">
                  <div className="form-group">
                ...
                <h3 style={{ margin: '1rem 0 0.75rem', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>Financial Settings</h3>
                <div className="form-row">
                  <div className="form-group">
