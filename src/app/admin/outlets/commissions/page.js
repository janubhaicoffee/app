...
export default function AdminCommissions() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [outletsRes] = await Promise.all([fetch('/api/admin/outlets', { headers })]);
      if (outletsRes.ok) {
  ...
  const totalPending = commissions
    ...
    .reduce((s, c) => s + Number(c.total_commission), 0);
    ...
    <div>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: '1.25rem', fontWeight: 700, color: '#856404' }}>
            ₹{totalPending.toLocaleString()}
          </div>
        </div>
        <div>
          <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Approved</span>
          <div style={{ fontSize: '1.25rem', fontWeight: 700, color: '#004085' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>
            ₹{(totalPending + totalApproved + totalPaid).toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div className="admin-toolbar">
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button
            ...
            onClick={() => setView(view === 'summary' ? 'list' : 'summary')}
          >
          ...
          {selected.size > 0 && (
            <>
              <button
                ...
                onClick={() => doAction('bulk_approve_commissions', Array.from(selected))}
              ...
              <button
                className="admin-btn sm"
                onClick={() => doAction('bulk_pay_commissions', Array.from(selected))}
                disabled={actionLoading}
                        ...
                        {c.status === 'pending' && (
                          <button
                            ...
                            onClick={async () => {
                              ...
                              fetchData(
                                await supabase.auth.getSession().then((s) => s.data.session),
                              );
