...
import {
  ...
  AlertTriangle, TrendingUp, Clock, RefreshCw, BarChart3
} from "lucide-react";
    ...
    <div>
      ...
      <div className="stats-grid">
        <div className="stat-card green">
          ...
          <p className="stat-value"><AnimatedNumber value={data.revenue} isCurrency={true} /></p>
          <p className="stat-sub">Lifetime sales</p>
        </div>
        <div className="stat-card">
          <h3><ShoppingCart size={14} style={{ display: 'inline', marginRight: 4 }} /> Total Orders</h3>
          <p className="stat-value"><AnimatedNumber value={data.orders} /></p>
          <p className="stat-sub">All time</p>
        </div>
        <div className="stat-card blue">
          <h3><Users size={14} style={{ display: 'inline', marginRight: 4 }} /> Customers</h3>
          <p className="stat-value"><AnimatedNumber value={data.customers} /></p>
          <p className="stat-sub">Registered accounts</p>
        </div>
        <div className="stat-card">
          <h3><Package size={14} style={{ display: 'inline', marginRight: 4 }} /> Products</h3>
          <p className="stat-value"><AnimatedNumber value={data.products} /></p>
          <p className="stat-sub">In catalog</p>
        </div>
        <div className="stat-card red">
          <h3><Star size={14} style={{ display: 'inline', marginRight: 4 }} /> Pending Reviews</h3>
          <p className="stat-value"><AnimatedNumber value={data.pendingReviews} /></p>
      ...
      <div className="charts-grid">
        <div className="admin-card">
          <div className="admin-card-header">
            <h2>Revenue <span style={{ fontWeight: 400, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>(Last 30 Days)</span></h2>
          ...
          <div style={{ width: '100%', height: 280 }}>
            <ResponsiveContainer>
              <AreaChart data={data.chartData}>
                ...
                <Tooltip formatter={(value) => [`₹${Number(value).toLocaleString('en-IN')}`, "Revenue"]} labelFormatter={(label) => `Date: ${label}`} />
                <Area type="monotone" dataKey="revenue" stroke="#2e7d32" strokeWidth={2} fill="url(#revenueGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="admin-card">
          <div className="admin-card-header">
            <h2>Orders <span style={{ fontWeight: 400, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>(Last 30 Days)</span></h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
        <div className="admin-card">
          <div className="admin-card-header">
            <h2>Recent Orders</h2>
            <Link href="/admin/orders" className="admin-btn admin-btn-sm">View All</Link>
          </div>
              ...
              <tbody>
                {data.recentOrders.slice(0, 10).map((order, i) => (
                  <tr key={order.id || i}>
                    ...
                    <td style={{ fontWeight: 600 }}>₹{Number(order.total || order.amount || 0).toLocaleString('en-IN')}</td>
                    <td>
        ...
        <div className="admin-card">
          <div className="admin-card-header">
            ...
            <Link href="/admin/inventory" className="admin-btn admin-btn-sm admin-btn-outline">Manage</Link>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {data.lowStockAlerts.slice(0, 8).map((item, i) => (
                <div key={item.id || i} style={{
                  ...
                  padding: '0.6rem 0.75rem', background: '#fff8f0',
                  borderRadius: '6px', border: '1px solid #fde6c8'
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                    <AlertTriangle size={16} style={{ color: '#f59e0b', flexShrink: 0 }} />
                    <div>
                      <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{item.name || item.product_name || item.title || 'Unknown Product'}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{item.variant || item.sku || ''}</div>
                    </div>
                  </div>
                  <span style={{
          ...
          <table className="admin-table">
            ...
            <tbody>
              {data.topProducts.slice(0, 8).map((product, i) => (
                <tr key={product.id || i}>
                  <td style={{ fontWeight: 700, color: 'var(--text-secondary)', width: 40 }}>{i + 1}</td>
                  <td style={{ fontWeight: 600 }}>{product.name || product.title || 'Product'}</td>
                  <td style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>{product.category || product.type || '-'}</td>
                  <td style={{ fontWeight: 600 }}>{product.sold || product.orders_count || product.quantity || 0}</td>
