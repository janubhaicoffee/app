...
export default function OutletComparison() {
  ...
  const outletMetrics = outlets.map(outlet => {
    const outletOrders = validOrders.filter(o =>
  ...
  if (loading) {
    return <div className="admin-loading"><div className="admin-spinner" /> Loading comparison data...</div>;
  }
      ...
      {selectedMetrics.length === 0 ? (
        ...
        <>
            ...
            <div style={{ width: '100%', height: 300 }}>
              <ResponsiveContainer>
                <LineChart data={selectedMetrics[0]?.chartData || []}>
                  ...
                  <Legend />
                  {selectedMetrics.map((m, i) => (
                    <Line key={m.id} type="monotone" dataKey="revenue" data={m.chartData}
                      name={m.name}
                      stroke={COLORS[i % COLORS.length]}
              ...
              <table className="admin-table">
                ...
                <tbody>
                  <tr>
                    ...
                    {selectedMetrics.map(m => (
                      <td key={m.id} style={{ fontWeight: 700, color: '#2e7d32' }}>{formatINR(m.revenue)}</td>
                    ))}
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    {selectedMetrics.map(m => (
                      <td key={m.id} style={{ fontWeight: 600 }}>{formatINR(m.avgOrder)}</td>
                    ))}
                  </tr>
                  <tr>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
            {selectedMetrics.map((m, i) => (
              <div key={m.id} className="admin-card" style={{ marginBottom: 0, borderLeft: `4px solid ${COLORS[i % COLORS.length]}` }}>
                <div style={{ fontWeight: 700, fontSize: '1rem', marginBottom: '0.5rem' }}>{m.name}</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', fontSize: '0.85rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>Revenue</span>
                    <span style={{ fontWeight: 700, color: '#2e7d32' }}>{formatINR(m.revenue)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
