        ...
        <div key={i} className="gallery-item">
          ...
          <div className="gallery-item-actions">
            <button type="button" onClick={() => {
              const arr = [...images]; const [m] = arr.splice(i, 1); arr.splice(Math.max(0, i - 1), 0, m); onChange(arr);
            }} disabled={i === 0}>◀</button>
            <button type="button" onClick={() => onChange(images.filter((_, j) => j !== i))}><Trash2 size={14} /></button>
            <button type="button" onClick={() => {
              const arr = [...images]; const [m] = arr.splice(i, 1); arr.splice(Math.min(arr.length, i + 1), 0, m); onChange(arr);
          ...
          <div className="variant-grid">
            <div className="form-group">
              ...
              <input type="text" value={v.name} onChange={e => {
                const arr = [...variants]; arr[i].name = e.target.value; onChange(arr);
              }} />
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              ...
              <input type="number" value={v.weight} onChange={e => {
                const arr = [...variants]; arr[i].weight = parseFloat(e.target.value) || 0; onChange(arr);
              }} />
            </div>
            <div className="form-group">
              <label>Price (₹)</label>
              <input type="number" value={v.price} onChange={e => {
                const arr = [...variants]; arr[i].price = parseFloat(e.target.value) || 0; onChange(arr);
              }} />
            </div>
            <div className="form-group">
...
export default function ProductEditorForm({ initialData, isNew }) {
  ...
  const handleNameChange = (val) => {
    setFormData(prev => ({
      ...prev,
      name: val,
      id: isNew ? val.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "") : prev.id
    }));
  };

  const generateAISEO = async () => {
    if (!formData.name) { setError("Enter a product name first"); return; }
    setError("");
    try {
      const { data: { session } } = await supabase.auth.getSession();
  ...
  const blendTotal = (parseInt(formData.arabica_pct) || 0) + (parseInt(formData.chicory_pct) || 0) + (parseInt(formData.robusta_pct) || 0);
  ...
  return (
    <div className="product-editor-page">
      <div className="admin-header">
        ...
        {!isNew && (
          <button type="button" className="admin-btn-outline" onClick={() => window.open(`/product/${initialData.id}`, "_blank")}>
            <Eye size={14} /> Preview
          </button>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="editor-form">
          ...
          <section className="form-section">
            ...
            <div className="form-row">
              <div className="form-group flex-1">
                ...
                <input required type="text" value={formData.name} onChange={e => handleNameChange(e.target.value)} />
              </div>
              <div className="form-group flex-1">
                <label>Slug (URL) *</label>
                <input required type="text" value={formData.id}
                  onChange={e => setFormData({ ...formData, id: e.target.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "") })}
                />
                <span className="form-hint">https://www.janubhai.com/product/{formData.id || "..."}</span>
              </div>
            </div>
            <div className="form-row">
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                {formData.status !== "published" && <span className="form-hint">Only published products are visible to customers</span>}
              </div>
          ...
          {formData.variants.length === 0 && (
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <input type="number" min="0" step="0.01" value={formData.compare_at_price} placeholder="Original price"
                  onChange={e => setFormData({ ...formData, compare_at_price: e.target.value })}
                />
                {formData.compare_at_price && parseFloat(formData.compare_at_price) > parseFloat(formData.price) && (
                  <span className="form-hint form-hint-sale">Sale — {Math.round((1 - parseFloat(formData.price) / parseFloat(formData.compare_at_price)) * 100)}% off</span>
                )}
              </div>
              <div className="form-group">
                <label>Stock</label>
          ...
          {formData.variants.length === 0 && (
            ...
            {(formData.arabica_pct || formData.chicory_pct || formData.robusta_pct) && (
              <div className="blend-total">
                Total: {blendTotal}%
                {blendTotal > 0 && blendTotal !== 100 && <span className="blend-warning"> (should be 100%)</span>}
              </div>
            )}
          </section>
          )}
        ...
        <div className="editor-footer">
          <button type="button" className="admin-btn-outline" onClick={() => router.push("/admin/products")}>Cancel</button>
      ...
      <style jsx global>{`
        .product-editor-page { max-width: 900px; }
        .admin-btn-back {
          background: none; border: 1px solid var(--border-color); border-radius: 6px;
          width: 36px; height: 36px; display: flex; align-items: center; justify-content: center;
          cursor: pointer; color: var(--text-secondary); transition: all 0.2s;
        }
        .admin-btn-back:hover { background: var(--primary-color); color: #fff; border-color: var(--primary-color); }
        .editor-error {
          background: #ffebee; color: #c62828; padding: 12px 16px; border-radius: 8px;
        ...
        .editor-footer {
          ...
          padding: 1rem 2rem; border-top: 1px solid var(--border-color); background: #faf8f5;
        }
        ...
        .checkbox-label {
          ...
          font-size: 0.85rem !important; text-transform: none !important; padding: 0.5rem 0; white-space: nowrap;
        }
        ...
        .image-preview { width: 80px; height: 80px; border-radius: 8px; object-fit: cover; border: 2px solid var(--border-color); }
        .image-preview-placeholder { width: 80px; height: 80px; border-radius: 8px; border: 2px dashed var(--border-color); display: flex; align-items: center; justify-content: center; font-size: 0.65rem; color: #ccc; text-align: center; }

        .gallery-manager { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; }
        .gallery-item { width: 72px; border: 1px solid var(--border-color); border-radius: 6px; overflow: hidden; }
        .gallery-item img { width: 100%; height: 60px; object-fit: cover; display: block; }
        .gallery-item-actions { display: flex; gap: 2px; padding: 2px; background: #faf8f5; justify-content: center; }
        .gallery-item-actions button { border: none; background: none; cursor: pointer; font-size: 0.65rem; padding: 2px 4px; color: #666; border-radius: 2px; }
        ...
        .variant-header {
          ...
          margin-bottom: 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid #e8e0d8;
        }
        .variant-header h4 { margin: 0; font-size: 1rem; color: var(--primary-color); }
        .variant-grid {
        ...
        .admin-btn-icon-danger:hover { background: #ffebee; }
