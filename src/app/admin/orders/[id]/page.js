...
export default function OrderDetailPage() {
  ...
  async function fetchOrder() {
    try {
      ...
      if (!res.ok) { setError("Failed to load order"); setLoading(false); return; }
    ...
    <div>
      <div className="admin-header">
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          <Link href="/admin/orders" className="admin-btn-outline" style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", textDecoration: "none" }}>
            &larr; Back
          </Link>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            ...
            {order.shipping_address && (
              <div>
                ...
                <p style={{ marginTop: "0.25rem", fontSize: "0.9rem", color: "var(--text-secondary)", whiteSpace: "pre-wrap" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            <div>
              <label className="form-group" style={{ margin: 0 }}>
                <span style={{ display: "block", fontWeight: 600, fontSize: "0.85rem", color: "var(--text-secondary)", marginBottom: "0.25rem" }}>
                  Status
                </span>
                <select
                  value={status}
...
function renderTimeline(order) {
  ...
  if (order.status === "processing" || order.status === "shipped" || order.status === "delivered") {
    events.push({ time: order.updated_at || order.created_at, label: `Status changed to ${order.status}` });
  }
  if (order.awb_number) {
    events.push({ time: order.updated_at || order.created_at, label: `AWB assigned: ${order.awb_number}` });
  }
  ...
  return (
    <div className="admin-card" style={{ marginTop: "1.5rem" }}>
      ...
      <div style={{ position: "relative", paddingLeft: "1.5rem" }}>
        ...
        />
        {events.map((ev, i) => (
          <div key={i} style={{ position: "relative", paddingBottom: "1rem", paddingLeft: "1.5rem" }}>
            <div
              style={{
                position: "absolute",
                left: "-1.25rem",
