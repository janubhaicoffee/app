...
export async function POST(request) {
  try {
    ...
    const { name, code, address, city, state, pincode, phone, email, opening_time, closing_time, status, settings } = body;
...
export async function PATCH(request) {
  try {
    const body = await request.json();
    const { id, name, code, address, city, state, pincode, phone, email, opening_time, closing_time, status, settings } = body;
    ...
    if (settings !== undefined) {
      ...
      if (setObj.rent !== undefined) updates.rent_monthly = setObj.rent ? parseFloat(setObj.rent) : 0;
      if (setObj.electricity !== undefined) updates.electricity_monthly = setObj.electricity ? parseFloat(setObj.electricity) : 0;
      if (setObj.water !== undefined) updates.water_monthly = setObj.water ? parseFloat(setObj.water) : 0;
      if (setObj.internet !== undefined) updates.internet_monthly = setObj.internet ? parseFloat(setObj.internet) : 0;
      if (setObj.cogs !== undefined) updates.cogs_percentage = setObj.cogs ? parseFloat(setObj.cogs) : 35;
    }
