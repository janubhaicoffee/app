  ...
  try {
    ...
    if (type === "outlet_stock") {
      const { data: products, error } = await supabaseAdmin
        .from("pos_products")
        .select("id, name, sku, current_stock, min_stock, price, is_available, outlet_id, outlet:outlets(name)")
        .order("name");
  ...
  try {
    ...
    if (action === "transfer_to_outlet") {
      ...
      await supabaseAdmin.from("audit_log").insert([{
        ...
        details: { outlet_id, quantity: qty, new_stock: newStock, notes: notes || "Warehouse transfer" }
      }]);
    ...
    if (action === "adjust_stock") {
      ...
      await supabaseAdmin.from("audit_log").insert([{
        ...
        entity_type: "pos_product",
        entity_id: product_id,
        details: { outlet_id: product.outlet_id, previous_stock: product.current_stock, new_stock: parseInt(new_stock), notes: notes || "Manual adjustment" }
      }]);
