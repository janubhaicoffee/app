  ...
  try {
    ...
    if (type === 'dashboard') {
      const [
        ...
        supabase
          .from('products')
          .select('id, name, stock, low_stock_threshold, track_inventory, image_url, status, price')
          .lt('stock', 10)
          .gt('stock', 0)
          .limit(5),
        supabase
    ...
    if (type === 'customer_detail') {
      ...
      const [customerRes, ordersRes, fbRes] = await Promise.all([
        ...
        supabase
          ...
          .eq('user_id', id)
          .order('created_at', { ascending: false }),
        supabase.from('customer_facebook_data').select('*').eq('customer_id', id).maybeSingle(),
      ]);
    ...
    if (type === 'analytics') {
      ...
      (orderItems || []).forEach((item) => {
        if (item.orders?.status !== 'cancelled') {
          ...
          if (!productSales[key])
            productSales[key] = { name: item.product_name, totalSold: 0, revenue: 0 };
    ...
    if (type === 'commission_summary') {
      ...
      let query = supabase
        .from('commission_transactions')
        .select(
          'outlet_id, status, total_commission, period_year, period_month, outlet:outlets(name)',
        )
    ...
    if (type === 'sourced_products') {
      ...
      return NextResponse.json({
        data: (products || []).map((p) => ({ ...p, already_linked: linkedIds.includes(p.id) })),
      });
    }
    if (type === 'staff') {
      const adminEmailsList = (process.env.SUPERADMIN_EMAILS || '')
        .split(',')
        .map((e) => e.trim().toLowerCase());
      const staffData = [];
...
export async function POST(request) {
  try {
    ...
    if (action === 'create_product') {
      ...
      if (error) throw error;
      await logAudit(supabase, adminEmail, 'create', 'product', insertPayload.id, {
        name: payload.name,
      });
    ...
    if (action === 'update_product') {
      ...
      if (newId && newId !== id) {
        const { data: existing } = await supabase
          ...
          .maybeSingle();
        if (existing)
          return NextResponse.json({ error: `Slug "${newId}" is already in use` }, { status: 409 });
        const { data: oldData } = await supabase.from('products').select('*').eq('id', id).single();
        const { error: insertError } = await supabase
          .from('products')
          .insert([{ ...oldData, ...payload, id: newId, updated_at: new Date().toISOString() }]);
    ...
    if (action === 'update_inventory') {
      ...
      await logAudit(supabase, adminEmail, 'update_inventory', 'product', id, {
        previousStock,
        newStock: new_stock,
        reason,
      });
    ...
    if (action === 'create_media') {
      const { error } = await supabase
        .from('media_files')
        .insert({ ...payload, uploaded_by: auth.user?.id });
      if (error) throw error;
      return NextResponse.json({ success: true });
    }
