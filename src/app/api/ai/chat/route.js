    ...
    if (process.env.PINECONE_API_KEY) {
      try {
        ...
        if (pineconeData.matches && pineconeData.matches.length > 0) {
          contextStr += pineconeData.matches
            .map((match) => match.metadata?.text || '')
            .join('\n\n');
        } else {
          contextStr +=
            'We are Janu Bhai Coffee, a premium Chikmagalur coffee brand based in Delhi. We deliver PAN India.';
        }
      } catch (err) {
      ...
      contextStr +=
        'We are Janu Bhai Coffee, a premium Chikmagalur coffee brand based in Delhi. We deliver PAN India. We sell Instant Coffee (70% coffee, 30% chicory) and Wholesale AAA Coffee Beans.';
    }
