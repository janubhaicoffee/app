...
export async function GET(request) {
  try {
    ...
    let query = supabaseAdmin.from("pos_shifts").select("*").order("opened_at", { ascending: false });
...
export async function POST(request) {
  try {
    ...
    if (!staff_id || !outlet_id) {
      return NextResponse.json({ error: "Missing required fields: staff_id, outlet_id" }, { status: 400 });
    }
    ...
    if (openShift) {
      return NextResponse.json({ error: "Staff member already has an open shift" }, { status: 400 });
    }
...
export async function PATCH(request) {
  try {
    ...
    const cashDifference = closingCash !== null ? parseFloat((closingCash - expectedCash).toFixed(2)) : null;
