...
export async function POST(request) {
  try {
    ...
    if (!outlet_id || number === undefined) {
      return NextResponse.json({ error: "Missing required fields: outlet_id, number" }, { status: 400 });
    }
    ...
    if (existing) {
      return NextResponse.json({ error: "Table number already exists in this outlet" }, { status: 409 });
    }
