  ...
  try {
    ...
    if (!outlet_id || !name || price === undefined) {
      return NextResponse.json(
        { error: 'Missing required fields: outlet_id, name, price' },
        { status: 400 },
      );
    }
      ...
      .insert([
        {
          ...
          commission_per_unit:
            commission_per_unit !== undefined ? parseFloat(commission_per_unit) : null,
        },
      ])
      .select()
      .single();
...
export async function PATCH(request) {
  try {
    ...
    if (commission_per_unit !== undefined)
      updates.commission_per_unit =
        commission_per_unit !== null ? parseFloat(commission_per_unit) : null;
...
export async function DELETE(request) {
  try {
    ...
    console.error('POS Products DELETE error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
