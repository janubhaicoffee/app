...
export async function GET(request) {
  try {
    ...
    let query = supabaseAdmin
      ...
      .select("*, orders(id, order_number, total_amount, customer_name, customer_email, customer_phone, created_at, shipping_address)")
...
export async function PATCH(request) {
  try {
    ...
    const updates = {};
    if (status) {
      if (!["pending", "confirmed", "preparing", "ready", "picked_up", "cancelled"].includes(status)) {
        return NextResponse.json({ error: "Invalid status" }, { status: 400 });
      }
      updates.status = status;
      if (status === "ready") updates.ready_at = new Date().toISOString();
      if (status === "picked_up") updates.picked_up_at = new Date().toISOString();
      if (status === "confirmed" || status === "preparing") updates.notified_at = new Date().toISOString();
    }
...
export async function POST(request) {
  try {
    const body = await request.json();
    const { order_id, outlet_id, order_number, customer_name, customer_phone, items_summary } = body;
