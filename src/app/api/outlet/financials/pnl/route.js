...
export async function GET(request) {
  try {
    ...
    if (!outletId || !startDate || !endDate) {
      return NextResponse.json({ error: "Missing required query params: outletId, startDate, endDate" }, { status: 400 });
    }
    ...
    const netProfit = parseFloat((grossProfit - laborCost - rentAllocated - totalExpenses).toFixed(2));
