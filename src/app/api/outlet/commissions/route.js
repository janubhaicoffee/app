  ...
  try {
    ...
    if (!outletId) {
      return NextResponse.json(
        { error: 'Missing required query param: outletId' },
        { status: 400 },
      );
    }
    ...
    if (!isSuperAdmin) {
      ...
      if (!staff) {
        return NextResponse.json(
          { error: 'Forbidden: not a staff member of this outlet' },
          { status: 403 },
        );
      }
    }
    ...
    if (isSummary) {
      ...
      const summary = (txns || []).reduce(
        (acc, row) => {
          ...
          if (row.status === 'pending') {
            acc.pending_total += amt;
            acc.pending_count += 1;
          } else if (row.status === 'approved') {
            acc.approved_total += amt;
      ...
      (monthly || []).forEach((r) => {
        ...
        if (!monthlyMap[key])
          monthlyMap[key] = { period_year: r.period_year, period_month: r.period_month, total: 0 };
