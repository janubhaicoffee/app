  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { success: false, error: 'Missing required order data or payment signatures' },
        { status: 400 },
      );
    }
    ...
    const isCOD =
      paymentId && paymentId.startsWith('COD_') && razorpaySignature === 'COD_SIGNATURE';
    ...
    if (!orderRow) {
      const orderNumber = isCOD
        ? `JB-COD-${Math.floor(Date.now() / 1000)}`
        : `JB-${Math.floor(Date.now() / 1000)}`;
