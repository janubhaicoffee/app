...
export async function POST(request) {
  try {
    const { outletCode, items, customerName, customerPhone, notes, paymentMode, userId, guestCount } = await request.json();
    ...
    if (custErr) {
      ...
      if (existing) {
        ...
        await supabaseAdmin.from('customers').insert({
          ...
          visit_count: 1, last_visit_at: new Date().toISOString(),
        });
      }
    }
