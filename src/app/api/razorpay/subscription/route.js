...
export async function POST(req) {
  try {
    ...
    // 1. Create a dynamic plan for the specific cart amount
    // Razorpay requires amount in paise.
    const plan = await razorpay.plans.create({
      period: frequency, // "weekly" or "monthly"
      interval: 1,
      item: {
        name: `Coffee Subscription - ${frequency}`,
        amount: amount * 100,
        currency: 'INR',
        description: 'Dynamic cart total subscription',
      },
    });

    // 2. Create a subscription tied to this plan
    const subscription = await razorpay.subscriptions.create({
      plan_id: plan.id,
      customer_notify: 1,
      total_count: frequency === 'weekly' ? 52 : 12, // Subscribes them for 1 year
    });
