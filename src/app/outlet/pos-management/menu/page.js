...
export default function MenuPage() {
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'pos_categories' }, () => {
        fetchData();
      })
      .subscribe();
    return () => {
  ...
  const handleToggleAvailability = async (product) => {
    try {
      ...
      if (res.ok) {
        fetchData();
        setSuccess(`"${product.name}" ${product.is_available ? 'disabled' : 'enabled'}`);
    ...
    <div>
      <div className="outlet-page-header">
        <div>
          ...
          <p className="outlet-page-subtitle">
            {products.length} items &middot; {availableCount} available
          </p>
        </div>
      ...
      <div className="outlet-filter-bar">
        ...
        <span style={{ fontSize: 12, color: '#718096' }}>
          Changes sync in real-time to all POS terminals
        </span>
      </div>
                ...
                filteredProducts.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td>
                        {cat?.name || <span style={{ color: '#a0aec0' }}>Uncategorized</span>}
                      </td>
                      ...
                      <td>
                        <button
                          ...
                          title={p.available !== false ? 'Click to disable' : 'Click to enable'}
                        >
                          {p.available !== false ? (
                            <ToggleRight size={24} />
                          ) : (
