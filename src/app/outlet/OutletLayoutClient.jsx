...
export default function OutletLayoutClient({ children }) {
  ...
  if (mounted && isEmployee && isRestrictedPath) {
    return (
      ...
      >
        <h1 style={{ color: '#B71C1C', fontSize: '24px', fontWeight: 'bold', marginBottom: '8px' }}>
          Access Denied
        </h1>
        <p style={{ color: '#5D4037', fontSize: '14px', textAlign: 'center', maxWidth: '400px' }}>
          Your current staff profile role ({activeRole.toUpperCase()}) does not have permission to
          access this page.
        </p>
        <button
  ...
  return (
    <OutletGuard>
      <div className="outlet-layout-wrapper">
        <OutletSidebar />
        <main className="outlet-main-content">{children}</main>
      </div>
    </OutletGuard>
  );
}
