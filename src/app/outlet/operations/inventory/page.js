...
export default function InventoryPage() {
  ...
  useEffect(() => {
    ...
    channel
      .on('postgres_changes', { event: '*', schema: 'public', table: 'outlet_inventory' }, () => {
        fetchData();
      })
      .on(
        'postgres_changes',
    ...
    try {
      const res = await fetch('/api/outlet/inventory/transactions', {
        ...
        body: JSON.stringify({
          ...
          quantity: parseInt(adjustment.quantity) || 1,
  ...
  const lowStockItems = items.filter(
    (i) => i.stock !== null && i.threshold !== null && i.stock <= i.threshold,
  );
  const totalStock = items.reduce((s, i) => s + (parseInt(i.stock) || 0), 0);
  const totalValue = items.reduce(
    (s, i) => s + (parseInt(i.stock) || 0) * (parseFloat(i.cost_price) || 0),
    0,
  );
  ...
  return (
    <div>
      <div className="outlet-page-header">
        <div>
          ...
          <p className="outlet-page-subtitle">
            {items.length} items &middot; {lowStockItems.length} low stock
          </p>
        </div>
      ...
      <div className="outlet-stats-grid">
        ...
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon red">
          ...
          <div className="outlet-form-row">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => setNewItem((p) => ({ ...p, selling_price: e.target.value }))}
              />
            </div>
          </div>
          ...
          <table className="outlet-table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((item) => {
                  const isLow =
                    item.stock !== null && item.threshold !== null && item.stock <= item.threshold;
                  return (
                    <tr key={item.id}>
                      ...
                      <td>
                        {isLow ? (
                          ...
                          <span className="outlet-badge green">OK</span>
                        )}
                      </td>
                      <td>
                        <button
              ...
              <form onSubmit={(e) => handleAdjust(e, showAdjustForm)}>
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => setAdjustment((p) => ({ ...p, type: e.target.value }))}
                  >
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => setAdjustment((p) => ({ ...p, quantity: e.target.value }))}
                    required
                  />
                </div>
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => setAdjustment((p) => ({ ...p, unit_cost: e.target.value }))}
                  />
                </div>
                <div className="form-group">
                  <label>Notes</label>
                ...
                <button type="submit" className="outlet-btn primary">
                  Save Adjustment
                </button>
              </form>
            </div>
              ...
              {movements.length === 0 ? (
                ...
                movements.slice(0, 30).map((m) => (
                  <tr key={m.id}>
                    ...
                    <td>
                      <span
                        className={`outlet-badge ${m.type === 'purchase' ? 'green' : m.type === 'adjustment' ? 'blue' : 'red'}`}
                      >
                        {m.type}
                      </span>
                    </td>
