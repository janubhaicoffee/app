...
import {
  Users, Plus, Search, RefreshCw, Clock, UserCheck,
  Calendar, BadgeCheck, BadgeX
} from "lucide-react";
...
export default function StaffPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", role: "staff", hourly_rate: "", notes: "" });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!session) return;
      const { data: staffRec } = await supabase.from("outlet_staff").select("outlet_id").eq("user_id", session.user.id).maybeSingle();
      ...
      const [staffRes, attRes, schedRes] = await Promise.allSettled([
        ...
        fetch(`/api/outlet/staff/schedules${params}&startDate=${weekStart.toISOString().split("T")[0]}&endDate=${weekEnd.toISOString().split("T")[0]}`),
      ]);
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      const res = await fetch("/api/outlet/staff", {
        ...
        body: JSON.stringify({ outlet_id: outletId, ...form, hourly_rate: form.hourly_rate ? parseFloat(form.hourly_rate) : null }),
      });
  ...
  const handleClockAction = async (staffId, action) => {
    ...
    try {
      ...
      if (res.ok) { fetchData(); setSuccess(action === "clock_in" ? "Clocked in" : "Clocked out"); setTimeout(() => setSuccess(""), 2000); }
      else { const b = await res.json(); setError(b.error); }
    } catch {} finally { setClockingIn(null); }
  };
  ...
  if (loading) return <div className="outlet-loading"><div className="outlet-loading-spinner" /><p>Loading staff...</p></div>;
    ...
    <div>
      <div className="outlet-page-header">
        <div>
          ...
          <p className="outlet-page-subtitle">{activeStaff.length} active &middot; {onDuty} on duty</p>
        </div>
      ...
      <div className="outlet-stats-grid">
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon blue"><Users size={24} /></div>
          <div className="outlet-stat-info"><h3>{activeStaff.length}</h3><p>Active Staff</p></div>
        </div>
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon green"><UserCheck size={24} /></div>
          <div className="outlet-stat-info"><h3>{onDuty}</h3><p>On Duty Today</p></div>
        </div>
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon purple"><Calendar size={24} /></div>
          <div className="outlet-stat-info"><h3>{schedules.length}</h3><p>Week Schedules</p></div>
        </div>
      </div>

      <div className="outlet-tabs">
        <button className={`outlet-tab ${activeTab === "roster" ? "active" : ""}`} onClick={() => setActiveTab("roster")}>Roster</button>
        <button className={`outlet-tab ${activeTab === "attendance" ? "active" : ""}`} onClick={() => setActiveTab("attendance")}>Attendance</button>
        <button className={`outlet-tab ${activeTab === "schedule" ? "active" : ""}`} onClick={() => setActiveTab("schedule")}>Schedule</button>
      </div>
        ...
        <form className="outlet-form" onSubmit={handleAddStaff}>
          ...
          <div className="outlet-form-row">
            <div className="form-group">
              ...
              <input type="number" step="0.01" className="form-control" value={form.hourly_rate} onChange={e => setForm(p => ({ ...p, hourly_rate: e.target.value }))} />
            </div>
            <div className="form-group">
              <label>Notes</label>
          ...
          <button type="submit" className="outlet-btn primary" disabled={submitting}>{submitting ? "Adding..." : "Add Staff"}</button>
        </form>
      )}
        ...
        <div>
          <div className="outlet-filter-bar">
            <div style={{ position: "relative", flex: 1, maxWidth: 300 }}>
              ...
              <input className="form-control" style={{ paddingLeft: 32 }} placeholder="Search staff..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
              ...
              <table className="outlet-table">
                ...
                <tbody>
                  {filteredStaff.length === 0 ? (
                    <tr><td colSpan={6}><div className="outlet-empty"><p>No staff found</p></div></td></tr>
                  ) : filteredStaff.map(s => {
                      ...
                      <tr key={s.id}>
                        ...
                        <td>
                          <button className="outlet-btn sm" style={{
                            background: isClockedIn ? "#e53e3e" : "#38a169", color: "#fff",
                            border: "none", padding: "4px 10px", borderRadius: 4, cursor: "pointer"
                          }} onClick={() => handleClockAction(s.id, isClockedIn ? "clock_out" : "clock_in")} disabled={clockingIn === s.id}>
        ...
        <div className="outlet-card">
          ...
          <div className="table-responsive" style={{ maxHeight: 400 }}>
            <table className="outlet-table">
              ...
              <tbody>
                {attendance.length === 0 ? (
                  <tr><td colSpan={6}><div className="outlet-empty"><p>No attendance records today</p></div></td></tr>
                ) : attendance.map(a => (
                  <tr key={a.id}>
                    ...
                    <td>{a.clock_out ? new Date(a.clock_out).toLocaleTimeString() : <span className="outlet-badge green">Active</span>}</td>
                    <td>{a.hours_worked ? `${a.hours_worked}h` : "-"}</td>
                    <td><span className={`outlet-badge ${a.status === "present" ? "green" : a.status === "absent" ? "red" : "yellow"}`}>{a.status || "present"}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="outlet-card">
          ...
          <div className="table-responsive" style={{ maxHeight: 400 }}>
            <table className="outlet-table">
              ...
              <tbody>
                {schedules.length === 0 ? (
                  <tr><td colSpan={6}><div className="outlet-empty"><p>No schedules this week</p></div></td></tr>
                ) : schedules.map(s => (
                  <tr key={s.id}>
                    <td style={{ fontWeight: 600 }}>{s.outlet_staff?.name || "Unknown"}</td>
                    <td>{s.outlet_staff?.role || "-"}</td>
                    <td>{new Date(s.date).toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short" })}</td>
                    <td>{s.start_time}</td>
                    <td>{s.end_time}</td>
                    <td><span className={`outlet-badge ${s.status === "confirmed" ? "green" : s.status === "cancelled" ? "red" : "yellow"}`}>{s.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
