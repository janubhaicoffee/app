...
import {
  FileText, Calendar, Printer, RefreshCw, Download,
  Sun, DollarSign, ShoppingCart, Users
} from "lucide-react";
...
export default function DailyReport() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      const [ordersRes, expensesRes, dailyRes, staffRes, attRes] = await Promise.allSettled([
        ...
        fetch(`/api/outlet/financials/daily-sales?outletId=${oid}&startDate=${date}&endDate=${date}`),
        fetch(`/api/outlet/staff?outletId=${oid}`),
        fetch(`/api/outlet/staff/attendance?outletId=${oid}&date=${date}`),
      ]);

      let orders = [], expenses = [], dailySales = [];
      ...
      if (expensesRes.status === "fulfilled" && expensesRes.value.ok) {
        ...
        expenses = Array.isArray(data) ? data.filter(e => {
      ...
      orders.forEach(o => {
        ...
        if (paymentBreakdown[method] !== undefined) paymentBreakdown[method] += parseFloat(o.total_amount || 0);
      ...
      setReport({
        ...
        totalExpenses,
        netProfit: totalRevenue - parseFloat(dailyRecord.total_cogs || 0) - parseFloat(dailyRecord.total_labor || 0) - totalExpenses,
        paymentBreakdown,
        expenses,
        dailyRecord,
      });
  ...
  if (loading) return <div className="outlet-loading"><div className="outlet-loading-spinner" /><p>Generating report...</p></div>;
    ...
    <div>
      <div className="outlet-page-header outlet-print-hide">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="outlet-btn primary sm" onClick={handlePrint}><Printer size={14} /> Print</button>
          <button className="outlet-btn outline sm" onClick={fetchReport}><RefreshCw size={14} /></button>
        </div>
      </div>
      ...
      <div className="outlet-card" style={{ marginBottom: 20 }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          <h1 style={{ fontSize: 22, margin: 0, color: "#1a1a1a" }}>{outletName || "Janu Bhai Coffee"}</h1>
        ...
        <div className="outlet-stats-grid">
          <div className="outlet-stat-card">
            ...
            <div className="outlet-stat-info"><h3>{formatCurrency(report?.totalRevenue || 0)}</h3><p>Total Revenue</p></div>
          </div>
          <div className="outlet-stat-card">
            <div className="outlet-stat-icon blue"><ShoppingCart size={24} /></div>
            <div className="outlet-stat-info"><h3>{report?.totalOrders || 0}</h3><p>Total Orders</p></div>
          </div>
          <div className="outlet-stat-card">
            <div className="outlet-stat-icon purple"><Users size={24} /></div>
            <div className="outlet-stat-info"><h3>{attendance.filter(a => a.status === "present").length || 0}</h3><p>Staff Present</p></div>
          </div>
          <div className="outlet-stat-card">
            <div className="outlet-stat-icon orange"><Sun size={24} /></div>
        ...
        <div className="outlet-grid-2">
          <div>
            ...
            <table className="outlet-table" style={{ fontSize: 13 }}>
              <tbody>
                <tr><td>Total Orders</td><td style={{ fontWeight: 600, textAlign: "right" }}>{report?.totalOrders || 0}</td></tr>
                <tr><td>Paid Orders</td><td style={{ fontWeight: 600, textAlign: "right" }}>{report?.paidOrders || 0}</td></tr>
                <tr><td>Total Revenue</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.totalRevenue || 0)}</td></tr>
                <tr><td>Tax Collected</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.totalTax || 0)}</td></tr>
              </tbody>
            </table>
          </div>
          <div>
            ...
            <table className="outlet-table" style={{ fontSize: 13 }}>
              <tbody>
                <tr><td>Cash</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.paymentBreakdown?.cash || 0)}</td></tr>
                <tr><td>Card</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.paymentBreakdown?.card || 0)}</td></tr>
                <tr><td>UPI</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.paymentBreakdown?.upi || 0)}</td></tr>
                <tr><td>Other</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.paymentBreakdown?.other || 0)}</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="outlet-grid-2">
          <div>
            ...
            <table className="outlet-table" style={{ fontSize: 13 }}>
              <tbody>
                <tr><td>COGS</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.totalCogs || 0)}</td></tr>
                <tr><td>Labor Cost</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.totalLabor || 0)}</td></tr>
                <tr><td>Other Expenses</td><td style={{ fontWeight: 600, textAlign: "right" }}>{formatCurrency(report?.totalExpenses || 0)}</td></tr>
                <tr style={{ borderTop: "2px solid #e2e8f0" }}>
                  <td style={{ fontWeight: 700 }}>Net Profit</td>
                  <td style={{ fontWeight: 700, textAlign: "right", color: (report?.netProfit || 0) >= 0 ? "#38a169" : "#e53e3e" }}>
                    {formatCurrency(report?.netProfit || 0)}
                  </td>
                </tr>
              </tbody>
          ...
          <div>
            ...
            {report?.expenses?.length > 0 ? (
              ...
              <div className="outlet-empty"><p>No expenses recorded</p></div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          {attendance.filter(a => a.status === "present").length > 0 ? (
            <table className="outlet-table" style={{ fontSize: 13 }}>
              <thead>
                <tr><th>Name</th><th>Role</th><th>Clock In</th><th>Clock Out</th><th>Hours</th></tr>
              </thead>
              <tbody>
            ...
            <div className="outlet-empty"><p>No staff attendance recorded</p></div>
          )}
        </div>
        ...
        <div style={{ marginTop: 24, borderTop: "1px solid #e2e8f0", paddingTop: 16, textAlign: "center", fontSize: 12, color: "#a0aec0" }}>
