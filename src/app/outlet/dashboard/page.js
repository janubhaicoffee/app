  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      const [ordersRes, productsRes, invRes, dailyRes] = await Promise.allSettled([
        ...
        fetch(
          `/api/outlet/financials/daily-sales?outletId=${oid || ''}&startDate=${weekAgo}&endDate=${today}`,
        ),
      ]);
  ...
  return (
    <div>
      ...
      <div className="outlet-quick-actions">
        <button
          className="outlet-quick-action-btn"
        ...
        {['superuser', 'manager', 'superadmin'].includes(activeRole) && (
          <>
            <button
              className="outlet-quick-action-btn"
              onClick={() => router.push('/outlet/operations/expenses')}
            >
              <DollarSign size={24} /> Add Expense
            </button>
            <button
              className="outlet-quick-action-btn"
              onClick={() => router.push('/outlet/financials/pnl')}
            >
              <BarChart3 size={24} /> P&L Report
            </button>
            <button
              className="outlet-quick-action-btn"
              onClick={() => router.push('/outlet/operations/staff')}
            >
              <Users size={24} /> Staff
            </button>
            <button
              className="outlet-quick-action-btn"
              onClick={() => router.push('/outlet/analytics/sales')}
            >
      ...
      {['superuser', 'manager', 'superadmin'].includes(activeRole) && (
        <Accounting
          outletId={outletId}
          onTransactionAdded={triggerGlobalRefresh}
      ...
      {['superuser', 'manager', 'superadmin'].includes(activeRole) ? (
        <div className="outlet-grid-2">
          <Operations
            outletId={outletId}
            refreshTrigger={refreshKey}
            onTimezoneChanged={triggerGlobalRefresh}
          />
          <CustomerProfiling outletId={outletId} refreshTrigger={refreshKey} />
        </div>
      ) : (
        <div
          style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '24px', marginBottom: '2rem' }}
        >
          <CustomerProfiling outletId={outletId} refreshTrigger={refreshKey} />
        </div>
      )}
        ...
        <div className="outlet-card">
          ...
          {recentOrders.length === 0 ? (
            <div className="outlet-empty">
              <div className="outlet-empty-icon">
              ...
              {recentOrders.map((order) => (
                <li key={order.id} className="outlet-list-item">
                  <div className="outlet-list-item-info">
                    ...
                    <p>
                      {order.order_type} &middot; {new Date(order.created_at).toLocaleTimeString()}
                    </p>
                  </div>
              ...
              {lowStockItems.map((item) => (
                <li key={item.id} className="outlet-list-item">
                  <div className="outlet-list-item-info">
                    <h4>{item.name}</h4>
                    <p>
                      Stock: {item.stock ?? 'N/A'} &middot; Threshold: {item.threshold}
                    </p>
                  </div>
