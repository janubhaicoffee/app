...
export default function OutletPortalPage() {
  ...
  const handleVerifyOtp = async () => {
    ...
    if (token.length !== 6) { setLoginError("Enter 6-digit OTP"); return; }
    setSubmitting(true);
    setLoginError("");
    try {
      ...
      if (staff) {
        if (staff.user_id !== data.session.user.id) {
          await supabase.from('outlet_staff').update({ user_id: data.session.user.id }).eq('phone', formattedPhone);
        }
        localStorage.removeItem("outlet_login_attempts");
        localStorage.removeItem("outlet_login_lockout");
        setIsAuthenticated(true);
  ...
  return (
    <div className="portal-page">
      <div className="portal-card secure-border">
        ...
        {/* Login mode tabs */}
        <div style={{ display: 'flex', gap: '0', marginBottom: '24px', width: '100%', borderBottom: '2px solid var(--border-color, #D7CCC8)' }}>
          <button type="button" onClick={() => { setLoginMode("otp"); setLoginError(""); setOtpStep("phone"); setPhone(""); setOtp(["","","","","",""]); }}
            style={{ flex: 1, padding: '10px', background: loginMode === "otp" ? '#3E2723' : 'transparent', color: loginMode === "otp" ? '#fff' : '#5D4037', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '13px', transition: 'all 0.15s', borderRadius: '8px 8px 0 0' }}>
            <Phone size={14} style={{ marginRight: 6, display: 'inline' }} /> Phone OTP
          </button>
          <button type="button" onClick={() => { setLoginMode("email"); setLoginError(""); setEmail(""); setPassword(""); }}
            style={{ flex: 1, padding: '10px', background: loginMode === "email" ? '#3E2723' : 'transparent', color: loginMode === "email" ? '#fff' : '#5D4037', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '13px', transition: 'all 0.15s', borderRadius: '8px 8px 0 0' }}>
        ...
        {loginMode === "otp" ? (
          otpStep === "verify" ? (
            <form onSubmit={(e) => { e.preventDefault(); handleVerifyOtp(); }} className="portal-form">
              <p style={{ fontSize: '13px', textAlign: 'center', color: '#5D4037' }}>OTP sent to {phone}</p>
              <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                <label style={{ display: 'block', fontWeight: 600, marginBottom: '10px', fontSize: '13px' }}>6-Digit OTP</label>
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                  {otp.map((digit, index) => (
                    <input key={index} type="text" inputMode="numeric" maxLength={1} value={digit}
                      ...
                      id={`otp-${index}`}
                      style={{ width: '44px', height: '52px', textAlign: 'center', fontSize: '1.3rem', border: '2px solid var(--border-color, #D7CCC8)', borderRadius: '8px', fontFamily: 'monospace', fontWeight: 'bold', background: digit ? '#3E2723' : '#fff', color: digit ? '#fff' : '#3E2723', outline: 'none' }} />
                  ))}
                </div>
              </div>
              <button type="submit" className="btn-portal-primary" disabled={submitting || otp.join("").length !== 6}>
                {submitting ? "VERIFYING..." : "VERIFY OTP"}
              </button>
              <p style={{ marginTop: '12px', textAlign: 'center', fontSize: '0.85rem' }}>
                <button type="button" onClick={() => { setOtpStep("phone"); setOtp(["","","","","",""]); }}
                  style={{ background: 'none', border: 'none', color: '#B71C1C', cursor: 'pointer', fontWeight: 'bold', textDecoration: 'underline', fontSize: '0.85rem' }}>
                  Change phone number
                </button>
              </p>
            </form>
          ) : (
            <form onSubmit={(e) => { e.preventDefault(); handleSendOtp(); }} className="portal-form">
              <div className="portal-form-group">
                <label>Phone Number</label>
                <div className="portal-input-wrapper">
                  <Phone size={16} className="input-icon" />
                  <input type="tel" required disabled={submitting} value={phone}
                    onChange={(e) => setPhone(e.target.value)} placeholder="+91 98765 43210" maxLength={20} autoFocus />
                </div>
              </div>
          ...
          <form onSubmit={handleLogin} className="portal-form">
            <div className="portal-form-group">
              ...
              <div className="portal-input-wrapper">
                ...
                <input type="email" required disabled={lockoutSecs > 0 || submitting} value={email}
                  onChange={(e) => setEmail(e.target.value)} placeholder="you@janubhai.com" maxLength={80} autoComplete="off" />
              </div>
            </div>
            <div className="portal-form-group">
              ...
              <div className="portal-input-wrapper">
                ...
                <input type={showPassword ? "text" : "password"} required disabled={lockoutSecs > 0 || submitting} value={password}
                  onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" maxLength={60} autoComplete="off" />
                <button type="button" className="password-toggle" onClick={() => setShowPassword(!showPassword)} tabIndex={-1}>
                  {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
            </div>
            <button type="submit" className="btn-portal-primary" disabled={submitting || lockoutSecs > 0}>
              {submitting ? "AUTHENTICATING SECURELY..." : lockoutSecs > 0 ? `LOCKED OUT (${lockoutSecs}s)` : "SECURE LOG IN"}
            </button>
          </form>
        )}
      </div>
