...
export default function PnLPage() {
  ...
  const fetchPnL = useCallback(async () => {
    ...
    try {
      ...
      const prevStart = new Date(new Date(startDate).getTime() - rangeLen - 86400000).toISOString().split("T")[0];
      ...
      if (prevDailyRes.status === "fulfilled" && prevDailyRes.value.ok) {
        ...
        setPrevDailySales(Array.isArray(data) ? data.sort((a, b) => a.date?.localeCompare(b.date)) : []);
      }
    } catch (err) {
  ...
  const renderChange = (current, previous) => {
    ...
    const isUp = diff >= 0;
    return (
      <span style={{ color: isUp ? "#38a169" : "#e53e3e", fontSize: 12, display: "flex", alignItems: "center", gap: 2 }}>
  ...
  if (loading) return <div className="outlet-loading"><div className="outlet-loading-spinner" /><p>Loading P&L...</p></div>;
  ...
  return (
    <div>
      <div className="outlet-page-header">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <div className="outlet-tabs" style={{ margin: 0 }}>
            {PRESETS.map((p, i) => (
              <button key={p.label} className={`outlet-tab ${preset.label === p.label ? "active" : ""}`} onClick={() => setPreset(p)}>
                {p.label}
              </button>
            ))}
          </div>
          {preset.label === "Custom" && (
            <div style={{ display: "flex", gap: 4 }}>
              <input type="date" className="form-control" style={{ width: 140 }} value={customRange.start} onChange={e => setCustomRange(p => ({ ...p, start: e.target.value }))} />
              <input type="date" className="form-control" style={{ width: 140 }} value={customRange.end} onChange={e => setCustomRange(p => ({ ...p, end: e.target.value }))} />
            </div>
          )}
          <button className="outlet-btn outline sm" onClick={fetchPnL}><RefreshCw size={14} /></button>
        </div>
      </div>
      ...
      <div className="outlet-stats-grid">
        ...
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon blue"><TrendingDown size={24} /></div>
          <div className="outlet-stat-info">
        ...
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon orange"><TrendingDown size={24} /></div>
          <div className="outlet-stat-info">
      ...
      <div className="outlet-grid-2">
        <div className="outlet-card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #edf2f7", fontSize: 13, color: "#718096" }}>
        ...
        <div className="outlet-card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #edf2f7", fontSize: 13, color: "#718096" }}>
              ...
              <span>{pnl?.totalRevenue > 0 ? ((pnl.cogs / pnl.totalRevenue) * 100).toFixed(1) : 0}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="outlet-card" style={{ marginBottom: 24 }}>
        ...
        <div className="outlet-grid-2" style={{ marginBottom: 0 }}>
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "2px solid #e2e8f0", fontWeight: 600 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "14px 0", fontWeight: 700, fontSize: 18 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0" }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 20, color: (pnl?.profitMargin || 0) >= 0 ? "#38a169" : "#e53e3e" }}>
                {pnl?.profitMargin || 0}%
              </span>
            </div>
          </div>
          ...
          <div className="outlet-chart-body">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailySales}>
                ...
                <XAxis dataKey={(d) => new Date(d.date).toLocaleDateString("en-IN", { day: "numeric", month: "short" })} tick={{ fontSize: 11 }} />
                <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => "₹" + v} />
                <ReTooltip formatter={(v) => formatCurrency(v)} />
                <Bar dataKey="total_revenue" name="Revenue" fill="#38a169" radius={[4, 4, 0, 0]} />
                <Bar dataKey={(d) => parseFloat(d.total_expenses || 0) + parseFloat(d.total_labor || 0)} name="Expenses" fill="#e53e3e" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
