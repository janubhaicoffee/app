...
import {
  TrendingUp, RefreshCw, Calendar, Clock, Award
} from "lucide-react";
import {
...
const COLORS = ["#3182ce", "#38a169", "#dd6b20", "#e53e3e", "#805ad5", "#d69e2e", "#319795", "#e53e3e"];
...
export default function SalesAnalytics() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      let chart = Object.entries(dateMap)
        ...
        .map(([date, v]) => ({ date: new Date(date).toLocaleDateString("en-IN", { day: "numeric", month: "short" }), ...v }));
      if (period === "weekly") {
        const weeklyMap = {};
        chart.forEach(d => {
        ...
        chart = Object.entries(weeklyMap).map(([date, v]) => ({ date, revenue: v.revenue, orders: v.orders }));
      }
      ...
      setTopProducts(Object.entries(prodCount)
      ...
      setCategorySales(Object.entries(catMap)
        .sort(([, a], [, b]) => b - a)
      ...
      setPeakHours(Object.entries(hourCount)
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([hour, v]) => ({ hour, ...v })));
    } catch (err) {
  ...
  if (loading) return <div className="outlet-loading"><div className="outlet-loading-spinner" /><p>Loading analytics...</p></div>;
    ...
    <div>
      ...
      <div className="outlet-stats-grid">
        <div className="outlet-stat-card">
          ...
          <div className="outlet-stat-info"><h3>{formatCurrency(summary.totalRevenue)}</h3><p>Total Revenue</p></div>
        </div>
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon blue"><Calendar size={24} /></div>
          <div className="outlet-stat-info"><h3>{summary.totalOrders}</h3><p>Total Orders</p></div>
        </div>
        <div className="outlet-stat-card">
          <div className="outlet-stat-icon purple"><Award size={24} /></div>
          <div className="outlet-stat-info"><h3>{formatCurrency(summary.avgOrder)}</h3><p>Avg Order Value</p></div>
        </div>
      </div>
        ...
        <div className="outlet-chart-body">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={revenueData}>
              ...
              <Area type="monotone" dataKey="revenue" stroke="#3182ce" fill="#ebf8ff" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="outlet-chart-container">
          ...
          <div style={{ height: 300, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {categorySales.length === 0 ? (
              <div className="outlet-empty"><p>No data</p></div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={categorySales} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
