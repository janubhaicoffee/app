...
export default function CheckoutPage() {
  ...
  const [assistantMessage, setAssistantMessage] = useState(
    "Hi! I'm your Janu Bhai assistant. Need help with your address?",
  );
    ...
    const fetchShippingRates = async (pincodeStr) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setAssistantMessage(
            `It looks like our regular couriers might have trouble delivering to ${pincodeStr}. If you have an alternate nearby address or work address, that might ensure a faster delivery!`,
          );
          setShippingRate({
            shipping_cost: 60,
        ...
        setAssistantMessage(
          `We're having trouble fetching live shipping rates for ${pincodeStr}, but don't worry, you can still place your order!`,
        );
        setShippingRate({
          shipping_cost: 60,
          courier_name: 'Standard Postal Delivery',
    ...
    const fetchCityState = async (pincodeStr) => {
      try {
        ...
        if (data && data[0] && data[0].Status === 'Success') {
          ...
          setAssistantMessage(
            `Got it! Setting your location to ${fetchedCity}, ${fetchedState} based on your pincode.`,
          );
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (formData.address.length > 5 && formData.address.length < 15) {
      const timeout = setTimeout(() => {
        setAssistantMessage(
          'Could you add a nearby landmark or colony name? It helps our delivery partners find your door much faster!',
        );
      }, 1500);
      ...
      const options = {
        ...
        description:
          checkoutMode === 'subscription'
            ? `Coffee Subscription (${subFrequency})`
            : 'Order Checkout',
        ...
        <div className="checkout-master-card">
          <div className="checkout-form-section">
            ...
            <h2 className="section-label">
              {checkoutMode === 'gift' ? "Recipient's Shipping Details" : 'Shipping Details'}
            </h2>
            <form onSubmit={handlePayment} className="checkout-form">
              <div className="form-group">
                <label htmlFor="name">
                  {checkoutMode === 'gift' ? "Recipient's Full Name" : 'Full Name'}
                </label>
                <input
                  id="name"
                  type="text"
              ...
              {shippingLoading && (
                <p className="shipping-status calculating">Calculating shipping rates...</p>
              )}
              {shippingError && <p className="shipping-status error">{shippingError}</p>}
              {shippingRate && (
                <div className="shipping-status success">
                  <p>
                    <strong>Delivery via {shippingRate.courier_name}</strong>
                  </p>
                  <p className="shipping-days">
                    Estimated: {shippingRate.estimated_delivery_days} Days
                  </p>
                </div>
              )}
              ...
              <button
                type="submit"
                className="btn-primary full-width mt-20 pulse-hover"
                disabled={
                  shippingLoading ||
                  !!shippingError ||
