...
function runPercentageTests() {
  ...
  for (const tc of testCases) {
    ...
    if (diff < 1e-7) {
      console.log(`✅ PASS: ${tc.name} (val: ${tc.value}, min: ${tc.min}, max: ${tc.max}) -> Result: ${result}%`);
    } else {
      console.error(`❌ FAIL: ${tc.name} (val: ${tc.value}, min: ${tc.min}, max: ${tc.max}) -> Expected: ${tc.expected}%, Got: ${result}%`);
      failures++;
    }
  }
...
function analyzeProductClientCode() {
  ...
  if (hasTypeRange && hasMin && hasMax && hasValue && hasOnChange && hasOnFocus && hasOnBlur) {
    console.log("✅ PASS: Hidden native input overlay contains correct accessibility & state attributes.");
  } else {
  ...
  const hasHolographicShimmer = content.includes('holographic-reveal') && content.includes("left: ['-150%', '150%']");

  if (hasPerspective && hasAnimatePresenceMode && hasLockedDropRotateY && hasRevealedDropRotateY && hasHolographicShimmer) {
    console.log("✅ PASS: 3D Flip Card transition and holographic shimmer attributes are correctly configured.");
  } else {
    console.error("❌ FAIL: 3D card layout or holographic animation properties are missing/incorrect.");
    console.log({ hasPerspective, hasAnimatePresenceMode, hasLockedDropRotateY, hasRevealedDropRotateY, hasHolographicShimmer });
    pass = false;
  }
  ...
  const sleepDebtInit = content.includes('const sd = searchParams?.get("sleep_debt");') && content.includes('Number(sd) : 50');
  const workloadInit = content.includes('const wl = searchParams?.get("workload");') && content.includes('Number(wl) : 50');

  if (sleepDebtInit && workloadInit) {
    console.log("⚠️ WARNING: Query parameters 'sleep_debt' and 'workload' are read from URL without bounds check or clamping. User can inject arbitrary values.");
  }

  // Check if percentage calculation clamps to [0, 100]
  if (!content.includes('Math.min(100, Math.max(0')) {
    console.log("⚠️ WARNING: Percentage calculation does not clamp percentage values to [0, 100]. Customizer handles might render out-of-bounds if values are manipulated via URL parameters.");
  }
