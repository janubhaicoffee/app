...
export default function DeliveryIntegrations({ outletId, refreshTrigger }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const ordersRes = await fetch(
        `/api/integrations/orders?outletId=${outletId || ''}${partnerParam}`,
      );
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'delivered':
        return 'outlet-badge green';
      case 'preparing':
        return 'outlet-badge yellow';
      case 'declined':
        return 'outlet-badge red';
      default:
        return 'outlet-badge gray';
    }
  };
  ...
  if (error) {
    return (
      <div className="panel" data-testid="delivery-panel">
        <h2>Delivery Integrations</h2>
        <div
          className="offline-banner error-text"
          style={{
            padding: '8px 12px',
            background: '#fff5f5',
    ...
    <div className="panel" data-testid="delivery-panel">
          ...
          >
            ...
            {formSuccess && (
              <div
                ...
                style={{ color: 'green', fontSize: 12, marginBottom: 8 }}
              >
                {formSuccess}
              </div>
            )}
      ...
      {orders.length === 0 ? (
        <div
          ...
          data-testid="delivery-order-feed"
          ref={feedRef}
            ...
            return (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div
                    className="delivery-order-partner"
                    data-testid="delivery-order-partner"
                    style={{ fontWeight: 600 }}
                  >
                    {order.partner
                      ? order.partner.charAt(0).toUpperCase() + order.partner.slice(1)
                      : ''}
                  </div>
                  <div
