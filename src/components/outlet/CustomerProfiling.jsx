...
export default function CustomerProfiling({ outletId, refreshTrigger }) {
  ...
  let filtered = customers.filter((c) => {
    ...
    const matchLoyalty =
      loyaltyFilter === "all" ||
      (c.tier || "").toLowerCase() === loyaltyFilter.toLowerCase();
  ...
  return (
    <div className="panel" data-testid="customer-registry-panel">
      ...
      {/* Controls */}
      <div className="customer-controls" style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "12px" }}>
      ...
      {/* Customer Table */}
      {customers.length === 0 ? (
        <div className="empty-placeholder" data-testid="customer-registry-table" style={{ padding: "20px 0" }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  ...
                  >
                    {loyaltyFilter !== "all" ? "No customers found in this tier" : "No customers matched your search"}
                  </td>
                </tr>
              ) : (
                filtered.map((c) => (
