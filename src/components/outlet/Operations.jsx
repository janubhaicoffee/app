...
export default function Operations({ outletId, refreshTrigger, onTimezoneChanged }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const autoReorderItem = itemsList.find(item => item.auto_reorder && (item.stock || 0) <= (item.threshold || 0));
      if (autoReorderItem) {
    ...
    <div className="panel" data-testid="operations-panel">
              ...
              lowStockItems.map((item) => (
                <tr key={item.id} data-testid="stock-row">
                  <td style={{ fontWeight: 600 }}>
                    {item.name}
                    <span className="stock-alert-badge" data-testid="stock-alert-badge" style={{ marginLeft: 6 }}>
                      Low
                    </span>
                  </td>
      ...
      <div className="outlet-card" style={{ marginTop: 16 }}>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          ...
          {manualReorderStatus && (
            <span data-testid="reorder-status" style={{ fontWeight: 600, fontSize: 13, color: "#3182ce" }}>
              {manualReorderStatus}
            </span>
          )}
        </div>
      ...
      <div className="outlet-card" style={{ marginTop: 16 }}>
        ...
        {auditLogs.length === 0 ? (
          <div className="empty-placeholder" style={{ padding: "10px 0", color: "#a0aec0", textAlign: "center" }}>
            No audit logs found
          </div>
        ) : (
          <ul className="outlet-list" style={{ maxHeight: "200px", overflowY: "auto", padding: 0, listStyle: "none" }}>
            {auditLogs.map((log) => (
              <li
                key={log.id}
              ...
              >
                <div>
                  ...
                  <div style={{ color: "#718096", fontSize: "10px" }}>{log.admin_email} &middot; {new Date(log.created_at).toLocaleString()}</div>
                </div>
              </li>
            ))}
          </ul>
