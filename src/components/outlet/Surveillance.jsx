...
export default function Surveillance({ outletId, refreshTrigger }) {
  ...
  const handleToggleCamera = async (camera) => {
    try {
      ...
      if (res.ok) {
        setCameras((prev) =>
          prev.map((c) => (c.id === camera.id ? { ...c, active: !c.active } : c)),
        );
      }
    } catch (err) {
  ...
  return (
    <div className="panel" data-testid="surveillance-panel">
            ...
            >
              ...
              <div style={{ padding: '20px 0', textAlign: 'center' }}>
                {cam.active ? (
                  <Camera
                    size={32}
                    style={{ opacity: 0.7, margin: '0 auto 8px', display: 'block' }}
                  />
                ) : (
                  <MonitorOff
                    size={32}
                ...
                <p style={{ margin: 0, fontWeight: 600, fontSize: 13 }}>{cam.name}</p>
              </div>
              <button
      ...
      <div className="outlet-card" style={{ marginTop: 16 }}>
        ...
        <form onSubmit={handleAddCamera} data-testid="camera-form" className="outlet-form">
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            />
            <button type="submit" className="outlet-btn primary sm" data-testid="btn-add-camera">
              Add Camera
            </button>
          </div>
        </form>
      ...
      {/* Alert Feed */}
      <h3
        style={{ fontSize: 14, fontWeight: 600, color: '#4a5568', marginBottom: 10, marginTop: 20 }}
      >
