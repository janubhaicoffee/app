...
function Spinner({ message, className = "" }) {
  return (
    <div className={`flex flex-col items-center justify-center py-12 gap-3 text-[var(--text-secondary)] ${className}`}>
    ...
    <div className={`flex flex-col gap-3 ${className}`}>
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex flex-col gap-2">
          ...
          />
          <div className="h-3 bg-gray-100 rounded animate-pulse" style={{ width: `${Math.max(30, 80 - i * 10)}%` }} />
        </div>
      ))}
    </div>
  );
