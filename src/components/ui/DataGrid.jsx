  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16 text-[var(--text-secondary)]">
        <div className="w-7 h-7 border-2 border-[var(--border-color)] border-t-[var(--primary-color)] rounded-full animate-spin mr-3" />
        Loading...
      </div>
    );
  }
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-50/80">
              {columns.map((col) => (
                ...
                >
                  <div
                    className="inline-flex items-center gap-1.5 cursor-pointer select-none hover:text-[var(--primary-color)]"
                  ...
                  {col.filter && (
                    <div className="mt-1.5 relative" onClick={(e) => e.stopPropagation()}>
                      ...
                      <input
                        ...
                        className="w-full pl-7 pr-6 py-1 text-[0.7rem] border border-gray-200 rounded outline-none focus:border-[var(--primary-color)] bg-white"
                      />
                      {filters[col.key] && (
                        <button
                          onClick={() => {
          ...
          <tbody>
            {paginatedData.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="px-4 py-12 text-center text-sm text-[var(--text-secondary)]">
                  No data found.
                </td>
              </tr>
            ) : (
