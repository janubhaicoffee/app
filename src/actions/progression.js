...
export async function getUserProgression(accessToken) {
  ...
  try {
    ...
    // If profile doesn't exist, backfill
    if (
      profileError &&
      (profileError.code === 'PGRST116' || profileError.message?.includes('0 rows'))
    ) {
      const { data: newProfile, error: createError } = await client
        .from('user_profiles')
        .insert({ id: user.id, total_points: 15 }) // Welcome bonus points
        .select()
        .single();
    ...
    return {
      success: true,
      profile: profile || { id: user.id, total_points: 0 },
      ledger: ledger || [],
      user: {
        email: user.email,
        id: user.id,
...
export async function awardPoints(accessToken, points, actionType) {
  ...
  try {
    ...
    const newPoints = (profile?.total_points || 0) + Number(points);
    ...
    return { success: true, newPoints, ledgerEntry };
  } catch (err) {
